      ...
      <AnimatePresence>
        {options && (
          <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="glass-panel rounded-2xl p-7 w-full max-w-md border border-gold/20 relative overflow-hidden">
              ...
              <div className="relative flex gap-3 mt-6">
                <button onClick={() => close(false)} className="flex-1 py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                  {options.cancelText ?? 'Cancel'}
                </button>
                <button onClick={() => close(true)} className={`flex-1 py-2.5 rounded-lg text-sm font-bold border transition-colors ${options.danger ? 'bg-red-500/20 text-red-400 border-red-500/30 hover:bg-red-500/30' : 'bg-gold/20 text-gold border-gold/30 hover:bg-gold/30'}`}>
