...
export const StatsSection = () => {
  ...
  useEffect(() => {
    Promise.allSettled([getHorseRankings(), getTournaments(), getRaceSchedule(), getJockeyRankings()]).then(([horses, tournaments, races, jockeys]) => {
      const len = (r) => (r.status === 'fulfilled' ? (r.value?.result ?? []).length : null);
      ...
      if (tournaments.status === 'fulfilled') {
        const list = tournaments.value?.result ?? [];
        setLiveCount(list.filter((tour) => (tour.status ?? '').toLowerCase() === 'active').length);
      }
    });
  }, []);

  const items = [
    { value: stats.horses, label: 'Horses on leaderboard', sub: undefined },
    { value: stats.tournaments, label: 'Tournaments', sub: liveCount > 0 ? `${liveCount} ${'live now'}` : undefined },
    { value: stats.races, label: 'Races in schedule', sub: undefined },
    { value: stats.jockeys, label: 'Professional jockeys', sub: undefined },
  ];
    ...
    <section className="py-32 relative noise-bg">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {items.map((stat, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ delay: i * 0.1, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="relative group">
              ...
              <div className="glass-panel p-8 pb-10 text-center rounded-2xl relative z-10 min-h-[170px] flex flex-col items-center justify-center group-hover:border-gold/30 group-hover:-translate-y-2 transition-all duration-500">
                ...
                <div className="text-xs font-medium text-muted uppercase tracking-widest">{stat.label}</div>
                {stat.sub && (
        ...
        <p className="text-center text-[11px] text-muted/50 mt-6">{'Live data from Equestria system'}</p>
      </div>
    </section>
  );
};
