  ...
  useEffect(() => {
    getTournaments()
      .then((d) => {
        ...
        const upcoming = list.filter((x) => (x.status ?? '').toLowerCase() === 'upcoming').sort((a, b) => new Date(a.startDate ?? 0).getTime() - new Date(b.startDate ?? 0).getTime());
  ...
  return (
    <section id="tournaments" className="py-32 relative">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, scale: 0.97 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="relative rounded-3xl overflow-hidden glass-panel-elevated p-1">
          ...
          <div className="relative bg-navy rounded-[22px] overflow-hidden">
            ...
            <div className="relative p-10 md:p-16 flex flex-col lg:flex-row gap-12 items-center">
              <div className="flex-1 space-y-6">
                {isLive ? (
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-bold uppercase tracking-widest">
                    <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /> {'Active'}
                  </div>
                ) : (
                ...
                <div className="flex flex-wrap gap-6 text-sm text-muted">
                  {fmtDate(tournament.startDate) && (
                    <div className="flex items-center gap-2">
                      <Clock className="w-4 h-4 text-gold" /> {fmtDate(tournament.startDate)} → {fmtDate(tournament.endDate) ?? '—'}
                    </div>
                  )}
                  {rounds != null && (
                    <div className="flex items-center gap-2">
                      <Layers className="w-4 h-4 text-gold" /> {rounds} {'rounds'}
                    </div>
                  )}
                  <div className="flex items-center gap-2">
                    <Trophy className="w-4 h-4 text-gold" /> Tournament ID #{tournament.tournamentId ?? tournament.id}
                  </div>
                </div>

                {tournament.description && <p className="text-body leading-relaxed max-w-lg">{tournament.description}</p>}
              </div>
              ...
              <div className="w-full lg:w-[400px] glass-panel p-8 rounded-2xl relative shadow-2xl">
                ...
                <h3 className="text-lg font-serif text-white mb-6">{'Registration window'}</h3>

                {regProgress != null && (
                  <div className="space-y-4 mb-8">
                    <div className="flex justify-between text-sm">
                      <span className="text-muted font-medium">{regClosed ? 'Registration closed' : 'Registration time elapsed'}</span>
                    ...
                    <div className="h-2.5 bg-navy-light rounded-full overflow-hidden shadow-inner border border-glass-border">
                      <motion.div className={`h-full relative ${regClosed ? 'bg-red-400/70' : 'bg-gold'}`} initial={{ width: 0 }} whileInView={{ width: `${regProgress}%` }} viewport={{ once: true }} transition={{ duration: 1.4, ease: [0.16, 1, 0.3, 1], delay: 0.4 }} />
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="bg-navy/60 p-4 rounded-xl border border-glass-border text-center hover:border-gold/30 transition-colors">
                    ...
                    <div className="text-[10px] uppercase text-muted tracking-wider font-bold">{'Days left to register'}</div>
                  </div>
                  <div className="bg-navy/60 p-4 rounded-xl border border-glass-border text-center hover:border-gold/30 transition-colors">
                    <div className="text-3xl font-serif text-champagne mb-1 tabular">{rounds ?? '—'}</div>
                    <div className="text-[10px] uppercase text-muted tracking-wider font-bold">{'Rounds'}</div>
                  </div>
                </div>

                <button onClick={() => navigate('/register')} className="btn-gold w-full py-4 rounded-lg flex items-center justify-center gap-2 text-sm shadow-[0_0_15px_rgba(201,168,76,0.2)] group">
                  {regClosed ? 'View tournament' : 'Join now'}
