  ...
  return (
    <div className="flex items-center justify-between gap-3 px-5 py-3 border-t border-glass-border/60">
      ...
      <div className="flex items-center gap-1.5">
        <button onClick={() => onChange(page - 1)} disabled={page <= 1} className={`${btn} border-glass-border text-muted hover:text-white hover:bg-white/5`}>
          <ChevronLeft size={13} className="mx-auto" />
        </button>
        {nums.map((n) => (
          <button key={n} onClick={() => onChange(n)} className={`${btn} ${n === page ? 'border-gold/40 bg-gold/15 text-champagne' : 'border-glass-border text-muted hover:text-white hover:bg-white/5'}`}>
            {n}
          </button>
        ))}
        <button onClick={() => onChange(page + 1)} disabled={page >= totalPages} className={`${btn} border-glass-border text-muted hover:text-white hover:bg-white/5`}>
