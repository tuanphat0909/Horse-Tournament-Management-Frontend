...
export function CountdownTimer({ target, utc = true, hideWhenExpired = false, label = 'Remaining:', className = '' }) {
  ...
  const tone = expired ? 'text-red-400 bg-red-500/10 border-red-500/25' : urgent ? 'text-orange-400 bg-orange-500/10 border-orange-500/25 animate-pulse' : 'text-yellow-400 bg-yellow-500/10 border-yellow-500/25';
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs font-bold tabular-nums whitespace-nowrap shrink-0 ${tone} ${className}`}>
      ...
      {expired ? (
        ...
        <>
          {label} <span className="tracking-wide">{formatRemaining(diff)}</span>
        </>
      )}
    </span>
  );
