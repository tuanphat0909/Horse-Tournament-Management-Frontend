    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="relative flex-1 min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero
            title="Deposit"
            subtitle="Top up your wallet to place bets and enjoy race events"
          ...
          {/* Balance summary */}
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-xl p-5 border border-gold/15 flex items-center justify-between">
            <div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-panel rounded-xl p-8 relative overflow-hidden max-w-2xl mx-auto">
              ...
              <div className="grid grid-cols-4 gap-2">
                {QUICK_AMTS.map((amt) => (
                  <button
                    key={amt}
                    onClick={() => {
                      setQuickAmt(quickAmt === amt ? null : amt);
                      setUsdInput('');
                    }}
                    disabled={isLocked}
                    className={`py-3 rounded-xl text-sm font-bold border transition-all ${isLocked ? 'opacity-40 cursor-not-allowed bg-white/5 border-glass-border text-muted/30' : quickAmt === amt ? 'bg-gold/15 border-gold/40 text-gold shadow-lg shadow-gold/10' : 'bg-white/[0.03] border-glass-border text-muted hover:text-white hover:border-white/20'}`}
                  >
                    ${amt}
                  </button>
                ))}
