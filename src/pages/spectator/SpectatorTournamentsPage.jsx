...
export function SpectatorTournamentsPage() {
  ...
  const [filter, setFilter] = useState(isReferee ? 'assigned' : 'all');
  const [sortBy, setSortBy] = useState('newest');
  const [tournaments, setTournaments] = useState([]);
  const [assignedTournamentIds, setAssignedTournamentIds] = useState([]);
    ...
    if (isReferee) {
      getRefereeDashboard()
        .then((res) => {
          ...
          const ids = races.map((r) => r.tournamentId).filter((id) => id != null);
          setAssignedTournamentIds(Array.from(new Set(ids)));
        })
        .catch((err) => {
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title={'Tournaments'} subtitle={'All active tournaments'} imageUrl="/images/hero-spectator.jpg" imagePosition="center 50%" />

          <div className="flex items-center gap-2 flex-wrap">
            {(isReferee ? ['assigned', 'all', 'active', 'upcoming', 'completed'] : ['all', 'active', 'upcoming', 'completed']).map((s) => (
          ...
          {loading ? (
            <div className="text-center py-12 text-muted text-sm">{'Loading tournaments list...'}</div>
          ) : filtered.length === 0 ? (
                  ...
                  <motion.div key={tour.tournamentId ?? i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-panel rounded-2xl p-5 border border-glass-border hover:border-gold/25 transition-all group relative overflow-hidden text-left h-full flex flex-col">
                    ...
                    <div className="flex justify-between items-center gap-2 mb-3 min-h-[26px]">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isReferee && assignedTournamentIds.includes(tour.tournamentId) && <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded border border-gold/45 bg-gold/12 text-champagne shrink-0">★ {'My Assignment'}</span>}
                      </div>
                    ...
                    <div className="mt-auto pt-3 border-t border-glass-border/40 flex justify-end">
                      <button onClick={() => navigate(`/spectator/tournaments/${tour.tournamentId}`)} className="text-xs font-bold bg-white/5 hover:bg-white/10 text-white px-4 py-2 rounded-lg transition-colors border border-glass-border hover:border-gold/30">
                        {'View details'}
                      </button>
                    </div>
                  </motion.div>
