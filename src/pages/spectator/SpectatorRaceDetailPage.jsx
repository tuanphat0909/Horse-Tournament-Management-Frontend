...
import { formatUSD, formatDateTime, formatWinProbability } from '../../utils/format';
...
export function SpectatorRaceDetailPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [raceRes, bettingRes, balanceRes, publicEntriesRes, resultsRes] = await Promise.all([getRaceDetail(raceId), getRaceBettingInfo(raceId).catch(() => null), getBalance().catch(() => ({ result: 0 })), getRaceEntries(raceId).catch(() => null), getPublicRaceResults(raceId).catch(() => null)]);
      ...
      if ((!fetchedEntries || fetchedEntries.length === 0) && publicEntriesRes?.result) {
        ...
        fetchedEntries = rawEntries.map((pe) => ({
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1200px] mx-auto px-8 py-6 flex flex-col xl:flex-row gap-6">
          <div className="flex-1 space-y-6">
            ...
            {results && results.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-2xl p-6 border border-gold/30 relative overflow-hidden bg-gradient-to-br from-gold/10 via-amber-950/20 to-navy">
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-gold/20">
                  ...
                  <span className="px-3 py-1 bg-gold/20 text-gold border border-gold/30 text-xs font-bold rounded-full uppercase tracking-wider">Published</span>
                </div>
                    ...
                    return (
                      <div key={res.id || idx} className="flex items-center justify-between p-4 bg-navy/60 border border-gold/20 rounded-xl">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-gold/20 border border-gold/40 flex items-center justify-center text-gold font-bold">{idx === 0 ? <Trophy size={18} /> : idx === 1 ? <Medal size={18} /> : <Award size={18} />}</div>
                          <div>
            ...
            {entries.length === 0 ? (
              <div className="glass-panel rounded-xl p-12 text-center text-muted">No horses participating in this race.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {entries.map((e) => {
                  ...
                  return (
                    <motion.div key={e.raceEntryId} whileHover={isBettingAllowed ? { scale: 1.01 } : {}} onClick={() => isBettingAllowed && setSelectedEntry(e)} className={`glass-panel rounded-xl p-4 border transition-all ${isBettingAllowed ? 'cursor-pointer hover:border-gold/50' : 'opacity-75 cursor-not-allowed border-glass-border'} ${isSelected ? 'border-gold bg-gold/5 ring-1 ring-gold/20' : 'border-glass-border'}`}>
          ...
          <AnimatePresence>
            {selectedEntry && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="w-full xl:w-96 shrink-0">
                <div className="glass-panel rounded-2xl p-6 border border-gold/30 sticky top-6">
                  ...
                  {!isBettingAllowed && (
                    <div className="mb-6 bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-3 rounded-lg text-sm flex items-start gap-2">
                      ...
                      {isLocked ? 'Your account is locked or inactive. Betting is disabled!' : invalidTournamentStatuses.includes(tournamentStatusKey) ? 'Tournament has ended, betting is disabled.' : 'Betting is not allowed for this race currently.'}
                    </div>
                  )}
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <div className="flex justify-between items-center px-1">
                      ...
                      <span className={`text-sm font-bold tabular-nums ${amount > balance ? 'text-red-400' : 'text-emerald-400'}`}>{formatUSD(balance)}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 pt-4 border-t border-glass-border/40 mb-6">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white tabular-nums">{formatUSD(amount)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted">Expected Payout:</span>
                      <span className="text-emerald-400 font-medium tabular-nums">+{formatUSD(potentialProfit)}</span>
                    </div>
                    <div className="flex justify-between text-base font-bold">
                      <span className="text-white">Total Return:</span>
                      <span className="text-gold tabular-nums">{formatUSD(totalReturn)}</span>
                    </div>
                  </div>
