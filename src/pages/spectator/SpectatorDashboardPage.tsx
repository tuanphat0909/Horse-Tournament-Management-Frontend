...
import { getNotifications, getRaceSchedule, getTournaments, getLiveRaces } from '../../api/publicService';
...
export function SpectatorDashboardPage() {
  ...
  useEffect(() => {
    ...
    getLiveRaces()
      .then(d => setLiveRaces((d as any)?.result ?? (Array.isArray(d) ? d : [])))
      .catch(() => setLiveRaces([]))
        ...
        <main className="relative z-10 max-w-400 mx-auto px-8 py-6 space-y-6">
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-5 gap-4">
            {[
              ...
              { title: 'Đang diễn ra', value: liveLoading ? '—' : String(liveRaces.length), trend: 'Live ngay', icon: Activity, color: 'text-red-400', bg: 'from-red-500/15 to-red-900/20', path: '/spectator/live' },
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              {liveLoading ? (
                ...
                <div className="space-y-2">
                  {liveRaces.map((r: any, i: number) => (
                    <div key={r.id ?? r.raceId ?? i} className="flex items-center gap-3 p-3.5 rounded-xl bg-red-500/5 border border-red-500/20 hover:border-red-500/40 transition-all group">
                      <div className="w-2 h-2 rounded-full bg-red-400 animate-pulse shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-white group-hover:text-champagne transition-colors truncate">{r.name ?? '—'}</div>
                        <div className="text-[11px] text-muted">{r.tournamentName ?? ''}{r.distanceMeter ? ` • ${r.distanceMeter}m` : ''}</div>
                      </div>
                      <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 shrink-0">Live</span>
                    </div>
                  ))}
                </div>
              )}
              <div className="mt-4 relative z-10">
                ...
                {upcoming.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {upcoming.map((u, i) => (
                      <div key={i} className="flex items-center gap-3 p-3.5 rounded-xl bg-white/2 border border-glass-border hover:border-gold/30 hover:bg-gold/4 transition-all group">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-white/4 border border-glass-border text-champagne shrink-0">{u.distanceMeter ? `${u.distanceMeter}m` : 'Sắp tới'}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {bets.length === 0 ? (
                ...
                <div className="space-y-3 relative z-10">
                  {bets.slice(0, 5).map((b, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between gap-3 p-3.5 rounded-xl bg-white/2 border border-glass-border hover:border-gold/30 hover:bg-gold/4 transition-all group">
                        <div>
                        ...
                        <div className="text-right">
                          ...
                          {(b.potentialPayout ?? b.actualPayout) != null && <div className="text-xs text-gold font-bold">+{Number(b.potentialPayout ?? b.actualPayout).toLocaleString()} coins</div>}
                        </div>
                      </div>
                    );
                  })}
