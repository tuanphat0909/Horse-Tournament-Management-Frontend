...
export function SpectatorTournamentDetailPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([getTournamentDetail(tournamentId), getRaceSchedule()])
      .then(([tourRes, raceRes]) => {
        ...
        const filteredRaces = allRaces.filter((r) => String(r.tournamentId) === String(tournamentId));
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1200px] mx-auto px-8 py-6 space-y-6">
          <Link to="/spectator/tournaments" className="inline-flex items-center gap-2 text-sm text-muted hover:text-white transition-colors">
            <ArrowLeft size={16} /> {'Back to tournaments list'}
          </Link>
          ...
          {loading ? (
            ...
            <div className="glass-panel p-8 text-center text-muted">{'Tournament not found.'}</div>
          ) : (
            <>
              {/* Tournament Header */}
              <div className="glass-panel rounded-2xl p-8 border border-glass-border relative overflow-hidden">
                ...
                <div className="flex items-center gap-6 text-sm text-muted mt-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {'End:'} {formatDateTime(tournament.endDate)}
                  </div>
                </div>
              </div>
              ...
              <h2 className="text-xl font-serif font-bold text-white">
                {'Races List'} ({races.length})
              </h2>

              {races.length === 0 ? (
                <div className="glass-panel rounded-xl p-12 text-center">
                  <div className="text-4xl opacity-40 mb-3">🏁</div>
                  <div className="text-muted text-sm">{'No races in this tournament yet.'}</div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {races.map((r, i) => {
                    ...
                    return (
                      <motion.div key={r.raceId} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-panel rounded-xl p-5 border border-glass-border hover:border-gold/30 transition-all flex flex-col">
                        ...
                        <div className="pt-4 border-t border-glass-border/50 flex gap-3">
                          <button onClick={() => navigate(`/spectator/races/${r.raceId}`)} className="flex-1 text-center text-xs font-bold bg-white/5 hover:bg-white/10 text-white px-4 py-2 rounded-lg transition-colors border border-glass-border">
                            {'View Details'}
                          </button>
                          {(() => {
                            ...
                            return isSpectator && canBet ? (
                              <button onClick={() => navigate(`/spectator/races/${r.raceId}`)} className="flex-1 text-center text-xs font-bold bg-gold hover:bg-gold-light text-navy px-4 py-2 rounded-lg transition-colors">
                                {'Bet Now'}
                              </button>
                            ) : null;
                          })()}
