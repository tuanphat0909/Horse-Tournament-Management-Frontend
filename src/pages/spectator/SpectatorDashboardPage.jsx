...
export function SpectatorDashboardPage() {
  ...
  useEffect(() => {
    getBalance()
      ...
      .catch(() => setBalance(0));
    getMyBets()
      .then((d) => setBets(d?.result ?? (Array.isArray(d) ? d : [])))
      .catch(() => setBets([]));
    getRaceSchedule()
      .then((d) => setUpcoming(d?.result ?? (Array.isArray(d) ? d : [])))
      .catch(() => setUpcoming([]));
    getTournaments()
  ...
  const pendingBets = bets.filter((b) => {
    ...
    return s !== 'win' && s !== 'won' && s !== 'lose' && s !== 'lost' && s !== 'correct' && s !== 'incorrect';
  }).length;
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <PageHero
            title={
              <>
                {'Welcome,'} <span className="italic text-champagne">{user?.fullName ?? 'Spectator'}</span>
              </>
            }
            subtitle={'Follow tournaments, place bets and manage your wallet'}
            ...
            badge={
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/25 text-purple-400 text-[10px] font-bold uppercase tracking-widest">
                <span className="w-1.5 h-1.5 rounded-full bg-purple-400 animate-pulse" /> {'Season 2026'}
              </div>
            }
            actions={
              <>
                <button onClick={() => navigate('/spectator/live')} className="btn-gold px-5 py-2 rounded-lg text-xs flex items-center gap-1.5 font-bold">
                  {'View live results'} <Eye size={13} />
                </button>
                <button
                  onClick={() => {
                    if (!isLocked) navigate('/spectator/predictions');
                  }}
                  disabled={isLocked}
                  className={`px-5 py-2 rounded-lg text-xs font-medium border transition-colors ${isLocked ? 'border-glass-border bg-white/5 text-muted/50 cursor-not-allowed' : 'text-champagne border border-gold/25 bg-gold/5 hover:bg-gold/10'}`}
                >
                  {'My Predictions'} {isLocked && '🔒'}
                </button>
              </>
            }
          />
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-5 gap-4">
            {[
              { title: 'Balance', value: balance.toLocaleString(), trend: `≈ $${(balance / 100).toFixed(2)}`, icon: Wallet, color: 'text-gold', bg: 'from-gold/15 to-amber-900/20', path: '/spectator/wallet' },
              ...
              { title: 'Notifications', value: String(notifCount), trend: 'Unread', icon: Bell, color: 'text-purple-400', bg: 'from-purple-500/15 to-purple-900/20', path: '/spectator/notifications' },
            ].map((m, i) => {
              ...
              return (
                <motion.div
                  ...
                  variants={child}
                  onClick={() => {
                    if (!isForbidden) navigate(m.path);
                  }}
                  className={`glass-panel rounded-xl p-5 relative overflow-hidden group transition-all ${isForbidden ? 'opacity-40 cursor-not-allowed border-glass-border' : 'cursor-pointer hover:border-gold/30'}`}
                  style={{ height: '130px' }}
                >
          ...
          <div className="grid grid-cols-[1fr_360px] gap-6">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center gap-3 mb-5">
                ...
                <h2 className="text-lg font-serif text-white">{'Active'}</h2>
                <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
                <button onClick={() => navigate('/spectator/live')} className="text-xs text-gold hover:text-champagne flex items-center gap-1 transition-colors font-medium shrink-0">
              ...
              <div className="mt-4 relative z-10">
                <div className="flex items-center gap-3 mb-3">
                  <h3 className="text-sm font-medium text-muted">{'Upcoming Tournaments'}</h3>
                  <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
                  <button onClick={() => navigate('/spectator/tournaments')} className="text-xs text-gold hover:text-champagne flex items-center gap-1 transition-colors font-medium shrink-0">
                    {'View all'} <ChevronRight size={12} />
                  </button>
                </div>
                {upcomingTournaments.length === 0 ? (
                  <div className="text-center py-6 text-muted text-sm">{'No tournaments yet'}</div>
                ) : (
                  <div className="space-y-2">
                    {upcomingTournaments.slice(0, 4).map((tour, i) => (
                      <button key={tour.tournamentId ?? i} onClick={() => navigate(`/spectator/tournaments/${tour.tournamentId}`)} className="w-full flex items-center gap-3 p-3.5 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/30 hover:bg-gold/5 transition-all group text-left">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[10px] text-muted">
                            {tour.startDate ? new Date(tour.startDate).toLocaleDateString('en-US') : ''}
                            {tour.endDate ? ` → ${new Date(tour.endDate).toLocaleDateString('en-US')}` : ''}
                          </div>
                        </div>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border shrink-0 ${tour.status === 'Active' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-blue-400 bg-blue-500/10 border-blue-500/20'}`}>{tour.status === 'Active' ? 'Active' : 'Upcoming'}</span>
                      </button>
                    ))}
                  </div>
                )}
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center gap-3 mb-5">
                ...
                <h2 className="text-base font-serif text-white">{'Recent Predictions'}</h2>
                <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
              </div>
              {bets.length === 0 ? (
                <div className="text-center py-8">
                  <div className="text-3xl opacity-40 mb-2">🎯</div>
                  <div className="text-muted text-sm">{'No predictions yet'}</div>
                </div>
              ) : (
                  ...
                  {bets.slice(0, 5).map((b, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between gap-3 p-3.5 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/30 hover:bg-gold/[0.04] transition-all group">
                        <div>
                          <div className="text-xs font-medium text-white">🐴 {b.horseName ?? 'Horse' + ' #' + b.horseId}</div>
