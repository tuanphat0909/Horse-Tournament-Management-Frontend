...
export function SpectatorLiveResultsPage() {
  ...
  const [expandedJockey, setExpandedJockey] = useState(null);
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="Results & Race Schedule" subtitle="Track races and upcoming schedule" imageUrl="/images/hero-spectator.jpg" imagePosition="center 50%" />
              ...
              <div className="space-y-3">
                {tournaments
                  ...
                  .map((tour, i) => {
                    ...
                    return (
                      <motion.div key={tour.tournamentId ?? i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="glass-panel rounded-xl p-5 border border-glass-border hover:border-gold/30 hover:bg-gold/[0.04] transition-all relative overflow-hidden group">
                        <div className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-gold/40 to-transparent pointer-events-none" />
                        <div className="relative z-10 flex items-center gap-4">
                            ...
                            <div className="flex flex-wrap items-center gap-3 text-xs text-muted mt-0.5">
                              {tour.startDate && (
                                <span className="flex items-center gap-1">
                                  <Clock size={10} /> {new Date(tour.startDate).toLocaleDateString('vi-VN')}
                                </span>
                              )}
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full border ${isActive ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-blue-400 bg-blue-500/10 border-blue-500/20'}`}>{isActive ? 'Active' : 'Upcoming'}</span>
                            <button onClick={() => navigate(`/spectator/tournaments/${tour.tournamentId}`)} className="text-xs font-bold px-3 py-1.5 rounded-lg bg-white/5 hover:bg-gold/10 border border-glass-border hover:border-gold/30 text-champagne transition-colors">
                              Detail
                            </button>
                          </div>
                        </div>
                ...
                {rankingsLoading ? (
                  ...
                  <div className="divide-y divide-glass-border">
                    {jockeyRankings.slice(0, 10).map((j, i) => {
                      ...
                      return (
                        <div key={jid}>
                          <button onClick={() => setExpandedJockey(isExpanded ? null : jid)} className={`w-full flex items-center gap-4 px-5 py-3.5 transition-all hover:bg-gold/5 text-left ${i === 0 ? 'bg-gold/5' : i < 3 ? 'bg-white/[0.03]' : ''}`}>
                ...
                {rankingsLoading ? (
                  ...
                  <div className="divide-y divide-glass-border">
                    {horseRankings.slice(0, 10).map((h, i) => {
                      ...
                      return (
                        <div key={hid}>
                          <button onClick={() => setExpandedHorse(isExpanded ? null : hid)} className={`w-full flex items-center gap-4 px-5 py-3.5 transition-all hover:bg-gold/5 text-left ${i === 0 ? 'bg-gold/5' : i < 3 ? 'bg-white/[0.03]' : ''}`}>
                            <div className={`w-7 h-7 rounded-full flex items-center justify-center font-serif font-bold text-sm border shrink-0 ${POS_STYLE[i + 1] ?? 'bg-white/5 text-muted border-glass-border'}`}>{i + 1}</div>
                            <div className="flex-1 min-w-0">
                          ...
                          <AnimatePresence>
                            {isExpanded && (
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="overflow-hidden">
                                <div className="px-5 py-3 bg-white/[0.02] border-t border-glass-border grid grid-cols-2 gap-2 text-xs text-muted">
                                  <span>
                                    Breed: <span className="text-champagne">{h.breed ?? '—'}</span>
                                  </span>
                                  <span>
                                    Owner: <span className="text-white">{h.ownerName ?? '—'}</span>
                                  </span>
                                  <span>
                    ...
                    {publishedRaces.map((race) => (
                      <div key={race.raceId || race.id} className="glass-panel p-5 rounded-xl border border-glass-border">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-bold">Published</span>
                        </div>
                        <div className="text-xs text-muted mb-4">
                          {race.tournamentName} • {new Date(race.raceDate || race.startTime).toLocaleDateString('vi-VN')}
                        </div>

                        {/* For a fully robust view we would call getRaceResults(race.raceId) but we can just display the overview for now */}
                        <div className="text-sm px-4 py-3 bg-white/[0.02] border border-glass-border rounded-lg text-white/80">Detailed results can be viewed on the race details page.</div>
                      </div>
                    ))}
                  </div>
                );
