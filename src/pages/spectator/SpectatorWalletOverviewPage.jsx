...
import { Wallet, TrendingUp, TrendingDown, History, CheckCircle, Clock, ArrowUpRight, Coins, Target } from 'lucide-react';
...
export function SpectatorWalletOverviewPage() {
  ...
  const [txFilter, setFilter] = useState('all');
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-4 gap-4">
            ...
            <motion.div variants={child} className="glass-panel rounded-xl p-5 relative overflow-hidden border border-gold/20">
              ...
              <div className="relative z-10 text-xl font-serif font-bold text-gold">{pageLoading ? '…' : `+${winTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}</div>
            ...
            <motion.div variants={child} className="glass-panel rounded-xl p-5 relative overflow-hidden border border-yellow-500/20">
              <div className="w-9 h-9 rounded-xl bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-center text-yellow-400 mb-3 relative z-10">
                <Clock size={16} />
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="glass-panel rounded-xl p-5 border border-yellow-500/15">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/spectator/wallet/withdraw')} className="text-[11px] font-bold text-gold hover:text-white transition-colors flex items-center gap-1">
            ...
            <div className="flex items-center justify-between text-xs text-muted mb-2">
              ...
              <span>
                Limit: <span className="text-white font-semibold">${SPECTATOR_DAILY_LIMIT.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span> USDT
              </span>
            </div>
            ...
            <div className="text-[10px] text-muted/60 mt-1.5 text-right">
              Remaining: <span className="text-emerald-400 font-semibold">${(SPECTATOR_DAILY_LIMIT - dailyUsedUsd).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span> USDT
            </div>
          </motion.div>
            ...
            <div className="relative z-10 flex items-center gap-3 mb-5 flex-wrap">
              ...
              <div className="flex items-center gap-1 shrink-0 flex-wrap">
                {FILTER_TABS.map(([f, label]) => (
                  <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${txFilter === f ? 'bg-gold/15 text-gold border border-gold/30' : 'text-muted hover:text-white'}`}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            {pageLoading ? (
              ...
              <div className="space-y-2.5 max-h-[520px] overflow-y-auto pr-1">
                {filtered.map((tx, i) => {
                  ...
                  return (
                    <motion.div key={tx.id ?? i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.035 }} className="flex items-center gap-3 p-3.5 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/30 hover:bg-gold/[0.04] transition-all">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[11px] text-muted">{tx.createdAt ? new Date(tx.createdAt).toLocaleString('en-US', { dateStyle: 'short', timeStyle: 'short' }) : '—'}</span>
                          {isPending && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">Pending</span>}
                        </div>
                      </div>
