...
import { getMedicalChecks, createMedicalCheck, getPendingRegistrations, getAssignedEntries, performRecheck, getUnhealthyHorses } from '../../api/vetService';
...
export function MedicalCheckPage() {
  ...
  const [unhealthyList, setUnhealthyList] = useState([]);
  ...
  const isEligibleForPass = parseFloat(temperature) >= 37.2 && parseFloat(temperature) <= 38.3 && parseInt(heartRate) >= 28 && parseInt(heartRate) <= 44 && parseFloat(weight) >= 300 && parseFloat(weight) <= 700 && dopingResult === 'Negative';
  ...
  function loadData() {
    setLoading(true);
    setError('');
    Promise.allSettled([getPendingRegistrations(), getAssignedEntries(), getMedicalChecks(), getUnhealthyHorses()]).then(([pendingRes, assignedRes, historyRes, unhealthyRes]) => {
  ...
  function openCreateForUnhealthy(uh) {
    setModalType('create');
  ...
  function handleSubmit(e) {
    ...
    if (medicalResult === 'Fail' && !failReason.trim()) {
      setError('FailReason is required when result is Fail.');
      return;
    }
    ...
    if (modalType === 'create') {
      createMedicalCheck({
        ...
        .then(() => {
          setSuccess('Inspection result saved!');
          setShowModal(false);
          loadData();
        })
        .catch((err) => {
      ...
      performRecheck({ registrationId: selectedRegId, ...base })
        .then((res) => {
          ...
          setSuccess(withdrawn ? `Recheck completed — Horse withdrawn from the race (Failed).` : `Recheck completed — Horse continues to compete (Passed).`);
          setShowModal(false);
          loadData();
        })
        .catch((err) => {
  ...
  const filteredPending = pendingList.filter((pc) => [pc.horseName, pc.tournamentName, pc.ownerName].some((s) => s?.toLowerCase().includes(search.toLowerCase())));
  const filteredAssigned = assignedList.filter((ae) => [ae.horseName, ae.raceName, ae.tournamentName, ae.ownerName].some((s) => s?.toLowerCase().includes(search.toLowerCase())));
  const filteredHistory = historyList.filter((mr) => [mr.horseName, mr.tournamentName, mr.checkedByName].some((s) => s?.toLowerCase().includes(search.toLowerCase())));
  const filteredRecovery = unhealthyList.filter((uh) => [uh.name, uh.breed, uh.ownerName].some((s) => s?.toLowerCase().includes(search.toLowerCase())));

  const TAB_BTN = (t, label, count) => (
    <button
      onClick={() => {
        setActiveTab(t);
        setSearch('');
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 relative min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="Horse Health Inspection" subtitle="Medical record, doping test and entry eligibility check - re-inspection of scheduled horses" imageUrl="/images/hero-referee.jpg" imagePosition="right 28%" />
                      ...
                      {filteredPending.map((pc) => {
                        ...
                        return (
                          <tr key={itemKey} className="hover:bg-white/[0.01] transition-colors">
                            ...
                            <td className="px-6 py-4">{pc.inspectionType === 'General' ? <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">🩺 Normal Health Check</span> : <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-blue-500/10 border border-blue-500/20 text-blue-400">🏆 Tournament: {cleanDisplayString(pc.tournamentName)}</span>}</td>
                    ...
                    <tbody className="divide-y divide-glass-border/40 text-sm text-white">
                      {filteredAssigned.map((ae) => (
                        <tr key={ae.raceEntryId} className="hover:bg-white/[0.01] transition-colors">
                          ...
                          <td className="px-6 py-4 text-muted whitespace-nowrap">{ae.raceDate ? new Date(ae.raceDate).toLocaleString('vi-VN', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—'}</td>
                          <td className="px-6 py-4 text-center font-mono font-bold text-champagne">{ae.laneNo}</td>
                          <td className="px-6 py-4">
                            {ae.lastMedicalResult ? (
                              <div>
                                <span className={`px-2 py-0.5 rounded text-xs font-semibold ${ae.lastMedicalResult === 'Pass' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>{ae.lastMedicalResult === 'Pass' ? 'Pass' : 'Fail'}</span>
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-0.5 rounded text-xs font-semibold border ${ae.raceEntryStatus === 'Withdrawn' || ae.raceEntryStatus === 'Disqualified' ? 'bg-red-500/10 text-red-400 border-red-500/20' : ae.raceEntryStatus === 'Confirmed' || ae.raceEntryStatus === 'Active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>{ae.raceEntryStatus || '—'}</span>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <button onClick={() => openRecheck(ae)} disabled={ae.raceEntryStatus === 'Withdrawn' || ae.raceEntryStatus === 'Disqualified'} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border border-cyan-500/30 bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          {activeTab === 'history' &&
            (loading ? (
              <div className="text-center py-12 text-muted">Loading history...</div>
                    ...
                    <tbody className="divide-y divide-glass-border/40 text-sm text-white">
                      {filteredHistory.map((mr) => (
                        <tr key={mr.id} className="hover:bg-white/[0.01] transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-0.5 rounded text-xs font-semibold ${mr.dopingResult === 'Negative' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>{mr.dopingResult === 'Negative' ? 'Negative' : 'Positive'}</span>
                          </td>
                          <td className="px-6 py-4">
                            <span className={`px-2 py-0.5 rounded text-xs font-semibold ${mr.medicalResult === 'Pass' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>{mr.medicalResult === 'Pass' ? 'Pass' : 'Fail'}</span>
                          </td>
                      ...
                      {filteredRecovery.map((uh) => (
                        <tr key={uh.horseId} className="hover:bg-white/[0.01] transition-colors">
                          ...
                          <td className="px-6 py-4 text-xs text-muted">
                            {calculateAge(uh.age)} yrs / {uh.gender}
                          </td>
                          <td className="px-6 py-4 text-muted">{uh.breed}</td>
                          <td className="px-6 py-4 text-muted">{uh.ownerName}</td>
                          <td className="px-6 py-4">
                            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-500/10 border border-red-500/20 text-red-400">{uh.healthStatus}</span>
                          </td>
                          <td className="px-6 py-4 text-right shrink-0">
                            <button onClick={() => openCreateForUnhealthy(uh)} className="px-3 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 transition-colors text-xs font-bold flex items-center gap-1.5 ml-auto">
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
              <div className="bg-[#0f172a] border border-glass-border rounded-xl w-full max-w-lg overflow-hidden shadow-2xl">
                <div className="px-6 py-4 border-b border-glass-border flex justify-between items-center bg-white/[0.02]">
                  ...
                  <button onClick={() => setShowModal(false)} className="text-muted hover:text-white text-xl font-bold">
                    ×
                  </button>
                </div>

                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  {modalType === 'recheck' && <div className="text-[11px] text-cyan-400/80 bg-cyan-500/5 border border-cyan-500/20 rounded-lg px-3 py-2">Re-inspect scheduled horses. If result is Fail, the system automatically withdraws the horse and sends notifications.</div>}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={dopingResult} onChange={(e) => setDopingResult(e.target.value)} className={INPUT_CLS}>
                        <option value="Negative" className="bg-[#0f172a]">
                          Negative (Negative)
                        </option>
                        <option value="Positive" className="bg-[#0f172a]">
                          Positive (Positive)
                        </option>
                      </select>
                    </div>
                    <div>
                      ...
                      <select value={medicalResult} onChange={(e) => setMedicalResult(e.target.value)} className={INPUT_CLS}>
                        <option value="Pass" disabled={!isEligibleForPass} className="bg-[#0f172a]" style={!isEligibleForPass ? { color: '#6b7280', cursor: 'not-allowed' } : {}}>
                          {isEligibleForPass ? 'Pass (✔ Eligible)' : 'Pass (⛔ Ineligible)'}
                        </option>
                        <option value="Fail" className="bg-[#0f172a]">
                          Fail (Fail)
                        </option>
                      </select>
                    </div>
                    ...
                    <div className="flex items-start gap-2.5 p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-red-300/80 leading-relaxed">
                          Temperature: 37.2-38.3°C &nbsp;•&nbsp; Heart Rate: 28-44 bpm &nbsp;•&nbsp; Weight: 300-700 kg &nbsp;•&nbsp; Doping: Negative
                          <br />
                  ...
                  {medicalResult === 'Fail' && (
                    <div>
                      ...
                      <select value={failReason} onChange={(e) => setFailReason(e.target.value)} className={INPUT_CLS}>
                        ...
                        <option value="Sick" className="bg-[#0f172a]">
                          Sick
                        </option>
                        <option value="Injured" className="bg-[#0f172a]">
                          Injured / Lameness
                        </option>
                        <option value="FailedMedicalReCheck" className="bg-[#0f172a]">
                          Failed Medical Re-inspection
                        </option>
                        <option value="VeterinaryDecision" className="bg-[#0f172a]">
                          Veterinary Decision
                        </option>
                        <option value="HorseInjury" className="bg-[#0f172a]">
                          Horse Injury
                        </option>
                      </select>
                    </div>
                  ...
                  <div className="pt-4 flex justify-end gap-3 border-t border-glass-border">
                    <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 border border-glass-border hover:bg-white/[0.05] rounded-lg text-sm text-muted hover:text-white transition-all">
                      Cancel
                    </button>
                    <button type="submit" disabled={loading || (medicalResult === 'Pass' && !isEligibleForPass)} title={medicalResult === 'Pass' && !isEligibleForPass ? 'Cannot save: Horse is not eligible for Pass' : ''} className="bg-gold hover:bg-gold/80 text-black font-bold px-4 py-2 rounded-lg text-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed inline-flex items-center gap-1.5">
                      {loading ? (
                        <>
