...
function isRegistrationOpen(tour) {
...
export function OwnerTournamentsPage() {
  ...
  useEffect(() => {
    Promise.all([getTournaments(), getMyRegistrations().catch(() => ({ result: [] }))])
      .then(([tourData, regData]) => {
  ...
  const myParticipatingTournaments = tournaments
    .filter((tour) => {
      ...
      return myRegistrations.some((reg) => reg.tournamentId === tour.tournamentId && !['rejected', 'disqualified', 'cancelled'].includes((reg.status ?? '').toLowerCase()));
    })
    .map((tour) => {
  ...
  const { paged: pagedMyTournaments, totalPages: myTotalPages, total: myTotal, page: safeMyPage } = paginate(myParticipatingTournaments, myPage, 6);
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title={'Tournaments'} subtitle={'Active and upcoming tournaments'} imageUrl="/images/hero-owner.jpg" imagePosition="center 5%" />
          ...
          {myParticipatingTournaments.length > 0 && (
            <div className="space-y-4 pt-2">
              <div className="flex items-center gap-2 border-b border-glass-border/30 pb-2">
                <span className="text-lg font-serif text-white font-bold flex items-center gap-2">🏆 {'Upcoming tournaments with my horses'}</span>
                <span className="bg-gold/10 text-gold border border-gold/20 text-xs px-2 py-0.5 rounded-full font-bold">{myParticipatingTournaments.length}</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {pagedMyTournaments.map((tour, i) => {
                  ...
                  return (
                    <motion.div key={`my-tour-${tour.tournamentId}`} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-panel rounded-2xl p-5 border border-gold/30 hover:border-gold/50 transition-all group relative overflow-hidden text-left h-full flex flex-col justify-between" style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.03) 0%, rgba(255,255,255,0.01) 100%)' }}>
                      ...
                      <div className="space-y-2 pt-3 border-t border-glass-border/40 text-xs">
                        <div className="flex flex-col gap-1.5 bg-white/[0.02] border border-glass-border/30 rounded-lg p-2.5">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-gold flex items-center gap-1.5">🐴 {'My registered horses:'}</span>
                          <div className="flex flex-wrap gap-1">
                            {tour.myHorses.map((h, idx) => {
                              const isAppr = (h.status ?? '').toLowerCase() === 'approved';
                              const isVet = (h.status ?? '').toLowerCase() === 'pendingvet';
                              const statusColor = isAppr ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300' : isVet ? 'bg-purple-500/10 border-purple-500/20 text-purple-300' : 'bg-yellow-500/10 border-yellow-500/20 text-yellow-300';
                              return (
                                <span key={idx} className={`text-[9px] font-medium px-2 py-0.5 rounded border ${statusColor}`}>
                                  {h.horseName} ({h.status})
                                </span>
                              );
                            })}
                          </div>
              ...
              {myTotalPages > 1 && <Pager page={safeMyPage} totalPages={myTotalPages} total={myTotal} onChange={setMyPage} />}
            </div>
          )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="ml-auto flex items-center gap-2 bg-white/[0.04] border border-glass-border focus-within:border-gold/40 rounded-lg px-3 py-2 w-56 transition-colors">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={'Search tournaments...'} className="bg-transparent text-sm text-white placeholder:text-muted/60 outline-none w-full" />
            </div>
                ...
                return (
                  <motion.div key={tour.tournamentId ?? i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-panel rounded-2xl p-5 border border-glass-border hover:border-gold/25 transition-all group relative overflow-hidden text-left h-full flex flex-col">
                    ...
                    <div className="flex justify-between items-center gap-2 mb-3 min-h-[26px]">
                      ...
                      {s !== 'cancelled' && s !== 'completed' && s !== 'finished' && (regNotStarted ? <CountdownTimer target={tour.registrationStartDate} utc={false} label="Reg. opens in:" /> : tour.registrationEndDate ? <CountdownTimer target={tour.registrationEndDate} utc={false} hideWhenExpired /> : null)}
                    </div>
                    <h3 className="text-lg font-serif text-white font-bold group-hover:text-champagne transition-colors mb-1 line-clamp-1">{tour.name}</h3>
                    <p className="text-xs text-muted/80 line-clamp-2 min-h-[32px] mb-3">{tour.description || 'No detailed description available.'}</p>
                              ...
                              {tour.prizes
                                ...
                                .map((p) => (
                                  <div key={p.id} className="bg-white/[0.03] border border-glass-border/40 rounded px-1 py-1">
                                    ...
                                    <div className="text-gold font-bold text-[10px] whitespace-nowrap tabular-nums">{formatUSD(Number(p.amount))}</div>
                                  </div>
                                ))}
                            </div>
                          ) : (
          ...
          {!loading && totalPages > 1 && <Pager page={safePage} totalPages={totalPages} total={total} onChange={setPage} />}
        </main>
      </div>
    </div>
  );
