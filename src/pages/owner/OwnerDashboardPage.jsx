...
import { Trophy, Calendar, ChevronRight, TrendingUp, Star, Clock, ShieldCheck, Flag, Wallet } from 'lucide-react';
...
export function OwnerDashboardPage() {
  ...
  useEffect(() => {
    getMyHorses()
      .then((d) => setHorses(d?.result ?? (Array.isArray(d) ? d : [])))
      .catch((err) => {
  ...
  useEffect(() => {
    getOwnerResults()
      .then((d) => {
        ...
        const upcoming = list.filter((r) => r.status !== 'Finished' && r.status !== 'Completed');
        ...
        const sum = (Array.isArray(list) ? list : []).reduce((acc, r) => acc + (Number(r?.prizeAmount) || 0), 0);
        setPrizeTotal(sum);
      })
      .catch((err) => {
        console.error(parseApiError(err));
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <PageHero
            title={
              <>
                {'Welcome,'} <span className="italic text-champagne">{user?.fullName ?? 'Horse Owner'}</span>
              </>
            }
            subtitle={'Your stable overview'}
            ...
            badge={
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gold/10 border border-gold/25 text-gold text-[10px] font-bold uppercase tracking-widest">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> {'Season in progress'}
              </div>
            }
            actions={
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    if (!isLocked) navigate('/owner/horses');
                  }}
                  disabled={isLocked}
                ...
                >
                  {'Manage Horses'} {isLocked ? '🔒' : <ChevronRight size={14} />}
                </button>
                <button onClick={() => navigate('/owner/wallet')} className="px-5 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 border border-gold/25 text-champagne bg-gold/5 hover:bg-gold/10 transition-all">
                  <Wallet size={13} /> {'My Wallet'}
                </button>
                <button
                  onClick={() => {
                    if (!isLocked) navigate('/owner/registrations');
                  }}
                  disabled={isLocked}
                  className={`px-5 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 border transition-all ${isLocked ? 'border-glass-border text-muted/50 cursor-not-allowed bg-white/5' : 'text-champagne border border-gold/25 bg-gold/5 hover:bg-gold/10'}`}
                >
                  {'Race Registration'} {isLocked && '🔒'}
                </button>
              </div>
            }
          />
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-4 gap-4">
            {[
              ...
              { title: 'Wallet Balance', value: walletBalance === null ? '…' : `${walletBalance.toLocaleString()} ¢`, trend: '', icon: Wallet, color: 'text-gold', bg: 'from-gold/15 to-amber-900/20', spark: SPARKS[1], to: '/owner/wallet' },
              ...
              { title: 'Prize Money', value: prizeTotal === null ? '…' : `${prizeTotal.toLocaleString()} ¢`, trend: '', icon: Trophy, color: 'text-gold', bg: 'from-gold/15 to-amber-900/20', spark: SPARKS[3], to: '/owner/results' },
            ].map((m, i) => {
              ...
              return (
                <motion.div
                  ...
                  variants={child}
                  onClick={() => {
                    if (!isForbidden) navigate(m.to);
                  }}
                  className={`glass-panel rounded-xl p-5 relative overflow-hidden group transition-colors ${isForbidden ? 'opacity-40 cursor-not-allowed border-glass-border' : 'cursor-pointer hover:border-gold/30'}`}
                  style={{ height: '140px' }}
                >
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.4 }} className="glass-panel rounded-xl p-6 flex flex-col relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center justify-between mb-5">
                <div className="flex items-center gap-3 flex-1 min-w-0 mr-4">
                  ...
                  <h2 className="text-lg font-serif text-white whitespace-nowrap">{'My Horses'}</h2>
              ...
              <div className="relative z-10 flex-1 space-y-2 overflow-y-auto max-h-[420px] pr-1 scrollbar-thin">
                {horsesLoading ? (
                  ...
                  <div className="text-center py-10">
                    ...
                    <div className="text-muted text-sm">{'No horses in your stable yet'}</div>
                  </div>
                ) : (
                  horses.map((h, i) => (
                    <div key={h.id ?? i} onClick={() => navigate('/owner/horses')} className="flex items-center gap-4 p-3.5 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/30 hover:bg-gold/[0.04] transition-all group cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-muted flex items-center gap-1.5 mt-0.5">
                          ...
                          <span>
                            {calculateAge(h.age)} {'years old'}
                          </span>
                        </div>
                      </div>
                      <div className="text-right shrink-0 hidden sm:block">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-muted/60 flex items-center gap-1 justify-end">
                          <Flag size={9} className="text-gold/50" /> {'Gender'}
                        </div>
                        <div className="text-xs text-champagne font-semibold">{h.gender === 'Male' ? 'Male' : h.gender === 'Female' ? 'Female' : '—'}</div>
                      </div>
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="text-right">
                          <div className="flex items-center gap-1 text-xs font-bold text-champagne">
                            <ShieldCheck size={11} /> {h.healthStatus ?? 'Healthy'}
                          </div>
                        </div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.4 }} className="glass-panel rounded-xl p-6 flex flex-col relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center justify-between mb-5">
                <div className="flex items-center gap-3 flex-1 min-w-0 mr-4">
                  ...
                  <h2 className="text-lg font-serif text-white whitespace-nowrap">{'Upcoming Schedule'}</h2>
                </div>
              ...
              <div className="relative z-10 flex-1 space-y-3 overflow-y-auto max-h-[420px] pr-1 scrollbar-thin">
                {scheduleLoading ? (
                  ...
                  <div className="glass-panel rounded-xl p-12 text-center relative overflow-hidden">
                    ...
                    <div className="text-4xl opacity-40 mb-3">📅</div>
                    <div className="text-muted text-sm">{'No upcoming races for your horses'}</div>
                  </div>
                ) : (
                  schedule.map((r, i) => (
                    <div key={r.raceId ?? i} onClick={() => navigate('/owner/tournaments')} className="relative overflow-hidden p-4 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/30 hover:bg-gold/[0.04] transition-all group cursor-pointer">
                      ...
                      <div className="mb-2 space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-1.5 items-center">
                          ...
                          {r.status && <span className="text-[9px] bg-blue-500/10 text-blue-400 font-bold px-2 py-0.5 rounded-full border border-blue-500/20 uppercase tracking-wider">{r.status}</span>}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-muted">
                        {r.raceDate && (
                          <span className="flex items-center gap-1">
                            <Clock size={11} className="text-gold/60" /> {formatDateTime(r.raceDate)}
                          </span>
                        )}
                      </div>
                    </div>
