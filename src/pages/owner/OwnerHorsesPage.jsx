...
export function OwnerHorsesPage() {
  ...
  // Lỗi trả về từ API (khác với lỗi validate từng ô của Formik)
  ...
  // Form thêm ngựa — Formik lo giữ giá trị + validate (createHorseSchema), API lỗi để riêng.
  const createFormik = useFormik({
    initialValues: INIT_CREATE,
    validationSchema: createHorseSchema,
    onSubmit: async (values) => {
      setCreateError('');
      try {
        await createHorse({ name: values.name, breed: values.breed, age: values.age, gender: values.gender });
        setShowCreate(false);
  ...
  // Form sửa ngựa — enableReinitialize để nạp lại giá trị mỗi khi chọn ngựa khác.
  const editFormik = useFormik({
    enableReinitialize: true,
    initialValues: editHorse
      ? { name: editHorse.name ?? '', breed: editHorse.breed ?? '', age: editHorse.age ? editHorse.age.split('T')[0] : '', gender: editHorse.gender ?? 'Male', healthStatus: editHorse.healthStatus || 'Healthy' }
      : INIT_EDIT,
    ...
    onSubmit: async (values) => {
      ...
      setEditError('');
      try {
        await updateHorse(editHorse.id, { name: values.name, breed: values.breed, age: values.age, gender: values.gender, healthStatus: values.healthStatus });
        setEditHorse(null);
  ...
  async function handleRequestRecovery(horseId) {
    setError('');
    try {
  ...
  async function openView(id) {
    ...
    try {
      ...
      if (horseData?.name) {
        const filtered = allResults.filter((r) => r.horseName === horseData.name);
  ...
  const filtered = horses
    ...
    .sort((a, b) => {
      switch (sortBy) {
        case 'ageAsc':
          return new Date(b.age ?? 0).getTime() - new Date(a.age ?? 0).getTime(); // sinh sau = trẻ hơn
        case 'ageDesc':
          return new Date(a.age ?? 0).getTime() - new Date(b.age ?? 0).getTime();
        case 'name':
        default:
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          {!loading && !error && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {filtered.map((h, i) => (
                <motion.div key={h.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} className="glass-panel rounded-2xl overflow-hidden border border-glass-border hover:border-gold/25 transition-all group relative h-full flex flex-col">
                  ...
                  <div className="p-5 relative flex-1 flex flex-col">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex gap-1.5">
                        <button onClick={() => openView(h.id)} className="p-1.5 rounded-lg bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-colors">
                          <Eye size={13} />
                        </button>
                        <button onClick={() => openEdit(h)} className="p-1.5 rounded-lg bg-gold/10 text-gold hover:bg-gold/20 transition-colors">
                          <Edit2 size={13} />
                        </button>
                        <button onClick={() => handleDelete(h.id)} disabled={deletingId === h.id} className="p-1.5 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors disabled:opacity-50">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </div>
                    {h.healthStatus && (
                      <div className="flex justify-between items-center text-[11px] text-muted font-medium mt-auto pt-3 border-t border-glass-border/60">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`font-semibold px-2 py-0.5 rounded-full text-[10px] border ${h.healthStatus === 'Healthy' || h.healthStatus === 'Good' ? 'bg-gold/[0.06] border-gold/20 text-champagne' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>{h.healthStatus}</span>
                          {h.healthStatus !== 'Healthy' && h.healthStatus !== 'Good' && (
                            <button onClick={() => handleRequestRecovery(h.id)} className="px-2 py-0.5 rounded bg-gold/10 hover:bg-gold/20 text-gold border border-gold/20 text-[10px] font-bold transition-all flex items-center gap-1" title="Request recovery check from Veterinarians">
            ...
            <form onSubmit={createFormik.handleSubmit} noValidate>
              <div className="space-y-4">
                <div>
                  ...
                  <input name="name" value={createFormik.values.name} onChange={createFormik.handleChange} onBlur={createFormik.handleBlur} placeholder="E.g.: Thunder King" className={INPUT} />
                  {createFormik.touched.name && createFormik.errors.name && <p className={ERR}>{createFormik.errors.name}</p>}
                </div>
                <div>
                  <label className={LABEL}>Breed *</label>
                  <input name="breed" value={createFormik.values.breed} onChange={createFormik.handleChange} onBlur={createFormik.handleBlur} placeholder="E.g.: Thoroughbred" className={INPUT} />
                  {createFormik.touched.breed && createFormik.errors.breed && <p className={ERR}>{createFormik.errors.breed}</p>}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className={LABEL}>Date of Birth *</label>
                    <input type="date" name="age" value={createFormik.values.age} onChange={createFormik.handleChange} onBlur={createFormik.handleBlur} className={INPUT} style={{ colorScheme: 'dark' }} />
                    {createFormik.touched.age && createFormik.errors.age && <p className={ERR}>{createFormik.errors.age}</p>}
                  </div>
                  <div>
                    <label className={LABEL}>Gender *</label>
                    <select name="gender" value={createFormik.values.gender} onChange={createFormik.handleChange} onBlur={createFormik.handleBlur} className={INPUT}>
                      <option value="Male">Male</option>
                      <option value="Female">Female</option>
                    </select>
                    {createFormik.touched.gender && createFormik.errors.gender && <p className={ERR}>{createFormik.errors.gender}</p>}
                  </div>
                </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button type="submit" disabled={createFormik.isSubmitting} className="flex-1 btn-gold py-2.5 rounded-lg text-sm font-bold disabled:opacity-60">
                  {createFormik.isSubmitting ? 'Saving...' : 'Save'}
                </button>
              </div>
            </form>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-gold/20">
            ...
            <form onSubmit={editFormik.handleSubmit} noValidate>
              <div className="space-y-4">
                <div>
                  <label className={LABEL}>Horse Name</label>
                  <input name="name" value={editFormik.values.name} onChange={editFormik.handleChange} onBlur={editFormik.handleBlur} className={INPUT} />
                  {editFormik.touched.name && editFormik.errors.name && <p className={ERR}>{editFormik.errors.name}</p>}
                </div>
                <div>
                  <label className={LABEL}>Breed</label>
                  <input name="breed" value={editFormik.values.breed} onChange={editFormik.handleChange} onBlur={editFormik.handleBlur} className={INPUT} />
                  {editFormik.touched.breed && editFormik.errors.breed && <p className={ERR}>{editFormik.errors.breed}</p>}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className={LABEL}>Date of Birth *</label>
                    <input type="date" name="age" value={editFormik.values.age} onChange={editFormik.handleChange} onBlur={editFormik.handleBlur} className={INPUT} style={{ colorScheme: 'dark' }} />
                    {editFormik.touched.age && editFormik.errors.age && <p className={ERR}>{editFormik.errors.age}</p>}
                  </div>
                  <div>
                    <label className={LABEL}>Gender</label>
                    <select name="gender" value={editFormik.values.gender} onChange={editFormik.handleChange} onBlur={editFormik.handleBlur} className={INPUT}>
                      <option value="Male">Male</option>
                      <option value="Female">Female</option>
                    </select>
                  </div>
                ...
                <div>
                  ...
                  <input value={editFormik.values.healthStatus} disabled className={`${INPUT} bg-navy/20 cursor-not-allowed opacity-75`} />

                  <p className="text-[11px] text-muted/70 italic mt-1">* Only Veterinarians can update health status via medical checkups.</p>
                </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button type="submit" disabled={editFormik.isSubmitting} className="flex-1 btn-gold py-2.5 rounded-lg text-sm font-bold disabled:opacity-60">
            ...
            {viewLoading ? (
              ...
              <>
                ...
                <div className="flex border-b border-glass-border mb-6">
                  <button onClick={() => setActiveTab('info')} className={`flex-1 pb-3 text-sm font-bold border-b-2 transition-all ${activeTab === 'info' ? 'border-gold text-gold' : 'border-transparent text-muted hover:text-white'}`}>
                    Information & Statistics
                  </button>
                  <button onClick={() => setActiveTab('history')} className={`flex-1 pb-3 text-sm font-bold border-b-2 transition-all ${activeTab === 'history' ? 'border-gold text-gold' : 'border-transparent text-muted hover:text-white'}`}>
                    Race History
                  </button>
                </div>
                        ...
                        <div key={idx} className="p-3.5 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/20 transition-all flex items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            <div className="text-right">
                              <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold ${r.finishPosition === 1 ? 'bg-gold/20 text-gold border border-gold/30' : r.finishPosition === 2 ? 'bg-slate-300/20 text-slate-300 border border-slate-300/30' : r.finishPosition === 3 ? 'bg-amber-700/20 text-amber-600 border border-amber-700/30' : 'bg-white/10 text-white'}`}>Rank {r.finishPosition}</span>
