    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="Race Results" subtitle="Race results and season achievements of owned horses" imageUrl="/images/hero-owner.jpg" imagePosition="center 5%" />
                      ...
                      {results.map((res, i) => (
                        <tr key={`${res.raceId}-${i}`} className="hover:bg-white/[0.01] transition-colors">
                          ...
                          <td className="px-6 py-4">
                            {res.status === 'Finished' ? (
                              <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-bold ${res.finishPosition === 1 ? 'bg-gold/15 text-gold border border-gold/30' : res.finishPosition === 2 ? 'bg-slate-400/15 text-slate-300 border border-slate-400/30' : 'bg-amber-600/15 text-amber-500 border border-amber-600/30'}`}>
                                {res.finishPosition === 1 && <Award size={12} />}
                                Rank {res.finishPosition}
                              </span>
                            ) : res.status === 'Live' || res.status === 'Running' || res.status === 'InProgress' ? (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 animate-pulse">Active</span>
                            ) : (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-bold bg-slate-500/15 text-slate-400 border border-slate-500/30">Not Started</span>
                            )}
                          </td>
