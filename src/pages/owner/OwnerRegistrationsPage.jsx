...
export function OwnerRegistrationsPage() {
  ...
  const [tab, setTab] = useState('pending_vet');
  const [search, setSearch] = useState('');
  const [showModal, setShowModal] = useState(false);
  // Lỗi API/nghiệp vụ (ngựa không đủ sức khoẻ) — khác lỗi validate từng ô của Formik
  ...
  async function load() {
    ...
    setError('');
    try {
      const [regData, horseData, tournamentData, propData] = await Promise.all([getMyRegistrations(), getMyHorses(), getTournaments(), getMyProposals().catch(() => ({ result: [] }))]);
  ...
  // Form đăng ký — Formik giữ giá trị + validate bắt buộc (registerHorseSchema);
  // luật nghiệp vụ "chỉ ngựa khoẻ mới được đăng ký" vẫn kiểm tra trong onSubmit.
  const registerFormik = useFormik({
    initialValues: { horseId: '', tournamentId: '' },
    ...
    onSubmit: async (values) => {
      ...
      try {
        await createRegistration({ horseId: Number(values.horseId), tournamentId: Number(values.tournamentId) });
        setShowModal(false);
  ...
  async function handleCancelRegistration(id, horseName, tournamentName) {
    ...
    try {
      ...
      showToast('Failed', parseApiError(err), 'error');
    }
  }
  ...
  const filtered = registrations.filter((r) => {
    ...
    const matchingContracts = proposals.filter((p) => String(p.horseId) === String(r.horseId) && String(p.tournamentId) === String(r.tournamentId));
    ...
    const contract = matchingContracts.find((p) => ['accepted', 'active', 'pending'].includes((p.status ?? '').toLowerCase())) || matchingContracts[0];
    ...
    return (r.horseName ?? '').toLowerCase().includes(q) || (r.tournamentName ?? '').toLowerCase().includes(q);
  });
  ...
  const filteredTournamentsForRegister = (registerFormik.values.horseId ? tournaments.filter((t) => !registrations.some((r) => String(r.horseId) === String(registerFormik.values.horseId) && r.tournamentId === t.tournamentId && normalizeStatus(r.status) !== 'rejected')) : tournaments)
    .filter((t) => {
      const status = (t.status ?? '').toLowerCase();
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="max-w-[1600px] mx-auto px-8 py-6 space-y-6 relative z-10">
          ...
          <div className="flex items-center gap-1 border-b border-glass-border pb-0">
            {[
              ['pending_vet', 'Vet Check'],
              ['pending_jockey', 'Hire Jockey'],
              ...
              <button key={t} onClick={() => setTab(t)} className={`px-5 py-3 text-sm font-medium border-b-2 -mb-px transition-all ${tab === t ? 'text-gold border-gold' : 'text-muted border-transparent hover:text-white'}`}>
                {label}
                <span className={`ml-2 px-1.5 py-0.5 rounded-full text-[11px] font-bold ${tab === t ? 'bg-gold/10 text-gold' : 'bg-white/5 text-muted'}`}>{counts[t]}</span>
              </button>
            ))}
            <div className="ml-auto mb-1 flex items-center gap-2 bg-white/[0.04] border border-glass-border rounded-lg px-3 py-1.5 w-56">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search horse, tournament..." className="bg-transparent text-sm text-white placeholder:text-muted/60 outline-none w-full" />
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {filtered.map((r, i) => {
                ...
                return (
                  <motion.div key={r.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="glass-panel rounded-xl p-5 flex items-center gap-5 border border-glass-border hover:border-gold/30 hover:bg-gold/[0.04] transition-all group relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-xs text-muted mt-1">
                        <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-white/[0.04] border border-glass-border text-champagne">
                          <Trophy size={10} className="text-gold/60" /> {r.tournamentName ?? `Tournament #${r.tournamentId}`}
                        </span>
                        {r.createdAt && (
                          <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-white/[0.04] border border-glass-border text-muted">
                            <Calendar size={10} className="text-gold/60" /> {r.createdAt}
                          </span>
                        )}
                        {/* Show jockey name inline */}
                        {(r.jockeyName || contract?.jockeyName) && (
                          <span className={`flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${resolvedStatus === 'accepted' || resolvedStatus === 'active' ? 'bg-blue-500/10 border-blue-500/20 text-blue-300' : resolvedStatus === 'rejected' || resolvedStatus === 'declined' ? 'bg-red-500/10 border-red-500/20 text-red-300' : 'bg-white/5 border-glass-border text-muted'}`}>
                    ...
                    <span onClick={handleBadgeClick} className={`relative z-10 text-[11px] font-bold px-3 py-1 rounded-full border shrink-0 transition-all flex items-center gap-1.5 ${customStatus.color}`} title={customStatus.clickable ? (customStatus.action === 'invite' ? 'Click to invite a jockey' : 'Click for details') : undefined}>
                      {customStatus.action === 'invite' && <ExternalLink size={10} />}
                      {customStatus.action === 'pending-modal' && <Clock size={10} />}
                      {customStatus.label}
                    </span>
                    {statusKey === 'pending' && (
                      <button onClick={() => handleCancelRegistration(r.registrationId, r.horseName, r.tournamentName)} className="relative z-10 p-2 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-colors shrink-0" title="Cancel registration">
                        <XCircle size={15} />
                      </button>
                    )}
      ...
      <AnimatePresence>
        {pendingModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setPendingModal(null)}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} onClick={(e) => e.stopPropagation()} className="glass-panel rounded-2xl p-8 w-full max-w-md border border-orange-500/20 relative">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-blue-500/5 border border-blue-500/15">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500/20 to-blue-900/20 border border-blue-500/20 flex items-center justify-center font-serif font-bold text-blue-300 text-lg">{(pendingModal.contract.jockeyName ?? 'J')[0]}</div>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setPendingModal(null)} className="flex-1 py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                  Close
                </button>
                <button
                  onClick={async () => {
            ...
            <form onSubmit={registerFormik.handleSubmit} noValidate>
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    name="horseId"
                    value={registerFormik.values.horseId}
                    onBlur={registerFormik.handleBlur}
                    onChange={(e) => {
                      const nextHorseId = e.target.value;
                      const isInvalid = registerFormik.values.tournamentId && registrations.some((r) => String(r.horseId) === String(nextHorseId) && String(r.tournamentId) === String(registerFormik.values.tournamentId) && normalizeStatus(r.status) !== 'rejected');
                      registerFormik.setValues({
                        horseId: nextHorseId,
                        tournamentId: isInvalid ? '' : registerFormik.values.tournamentId,
                      });
                    }}
                  ...
                  >
                    ...
                    {horses.map((h) => (
                      <option key={h.id} value={h.id} disabled={!isHealthy(h)}>
                        {h.name}
                        {!isHealthy(h) ? ` — unhealthy (${h.healthStatus})` : ''}
                      </option>
                    ))}
                  </select>
                  {registerFormik.touched.horseId && registerFormik.errors.horseId && <p className="text-xs text-red-400 mt-1">{registerFormik.errors.horseId}</p>}
                </div>
                <div>
                  <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-1.5">Select Tournament *</label>
                  <select name="tournamentId" value={registerFormik.values.tournamentId} onChange={registerFormik.handleChange} onBlur={registerFormik.handleBlur} className="w-full bg-navy/50 border border-glass-border rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-gold/40" disabled={!registerFormik.values.horseId}>
                    <option value="">{!registerFormik.values.horseId ? '-- Select Previous Horse --' : '-- Select Tournament --'}</option>
                  ...
                  {registerFormik.touched.tournamentId && registerFormik.errors.tournamentId && <p className="text-xs text-red-400 mt-1">{registerFormik.errors.tournamentId}</p>}
                  {registerFormik.values.horseId && filteredTournamentsForRegister.length === 0 && <div className="text-[11px] text-yellow-400 mt-1.5">This horse has already registered for all current tournaments.</div>}
                  {tournaments.length === 0 && <div className="text-[11px] text-yellow-400 mt-1.5">No tournaments available. An admin must create a tournament before horses can register.</div>}
                </div>
              ...
              <div className="flex gap-3 mt-6">
                <button type="button" onClick={closeModal} className="flex-1 py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                  Cancel
                </button>
                <button type="submit" disabled={registerFormik.isSubmitting} className="flex-1 btn-gold py-2.5 rounded-lg text-sm font-bold disabled:opacity-60">
                  {registerFormik.isSubmitting ? 'Submitting...' : 'Submit Registration'}
                </button>
              </div>
            </form>
          </motion.div>
