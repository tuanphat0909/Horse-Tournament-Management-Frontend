...
import { Wallet, Trophy, History, CheckCircle, ArrowUpRight, Coins, Users, TrendingUp } from 'lucide-react';
import { Sidebar } from '../../components/layout/Sidebar';
import { Topbar } from '../../components/layout/Topbar';
import { PageHero } from '../../components/layout/PageHero';
import { PageAmbience } from '../../components/layout/PageAmbience';
import { getOwnerWalletBalance, getOwnerWalletHistory } from '../../api/ownerService';
...
export function OwnerWalletOverviewPage() {
  ...
  async function loadAll() {
    ...
    setError('');
    try {
      const [balData, histData] = await Promise.all([getOwnerWalletBalance(), getOwnerWalletHistory()]);
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="relative flex-1 min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="glass-panel rounded-xl p-5 border border-blue-500/15">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/owner/wallet/withdraw')} className="text-[11px] font-bold text-gold hover:text-white transition-colors flex items-center gap-1">
            ...
            <div className="relative z-10 flex items-center gap-3 mb-5 flex-wrap">
              ...
              <div className="flex items-center gap-1 shrink-0 flex-wrap">
                {FILTER_TABS.map(([f, label]) => (
                  <button key={f} onClick={() => setTxFilter(f)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${txFilter === f ? 'bg-gold/15 text-gold border border-gold/30' : 'text-muted hover:text-white'}`}>
                    {label}
                  </button>
                ))}
              </div>
