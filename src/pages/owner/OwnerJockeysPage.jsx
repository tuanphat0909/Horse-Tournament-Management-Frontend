...
function contractBucket(status) {
...
export function OwnerJockeysPage() {
  ...
  const [busyJockeyIds, setBusyJockeyIds] = useState([]);
  const [loadingBusyJockeys, setLoadingBusyJockeys] = useState(false);

  function handleTournamentSelect(tId) {
    const selected = tournaments.find((t) => String(t.tournamentId) === String(tId));
    ...
    if (tId) {
      ...
      getBusyJockeysForTournament(Number(tId))
        .then((res) => {
          ...
          setBusyJockeyIds(ids.map((id) => Number(id)));
        })
  ...
  async function load(silent = false) {
    ...
    setError('');
    try {
      const [propData, horseData, jockeyData, tournamentData, regData] = await Promise.all([getMyProposals(), getMyHorses(), getJockeyRankings(), getTournaments(), getMyRegistrations().catch(() => ({ result: [] }))]);
  ...
  useEffect(() => {
    ...
    if (horseId && tournamentId) {
      ...
      const t = tournaments.find((t) => String(t.tournamentId) === String(tournamentId));
      setForm((prev) => ({
        ...prev,
        horseId,
        tournamentId,
  ...
  async function handleInvite() {
    ...
    try {
      await inviteJockeySchema.validate({ horseId: form.horseId, jockeyId: form.jockeyId, tournamentId: form.tournamentId, expiryHours: form.expirationHours }, { abortEarly: false });
    ...
    if (selectedTournament?.startDate && selectedTournament?.endDate) {
      ...
      if (start < tournamentStart || end > tournamentEnd) {
        setSubmitError(`Rental dates must be within tournament period: ${tournamentStart.toLocaleDateString('vi-VN')} - ${tournamentEnd.toLocaleDateString('vi-VN')}.`);
        return;
      }
    }
  ...
  async function handleCancelInvite(contractId) {
  ...
  const visibleProposals = proposals.filter((p) => {
    ...
    return (p.jockeyName ?? '').toLowerCase().includes(q) || (p.horseName ?? '').toLowerCase().includes(q);
  });

  const selectedInviteTournament = tournaments.find((t) => String(t.tournamentId) === String(form.tournamentId));

  const filteredHorsesForSelect = form.tournamentId
    ? horses.filter((h) => {
        const reg = registrations.find((r) => String(r.horseId) === String(h.id) && String(r.tournamentId) === String(form.tournamentId));
        ...
        const hasContract = proposals.some((p) => String(p.horseId) === String(h.id) && String(p.tournamentId) === String(form.tournamentId) && ['pending', 'accepted', 'active'].includes((p.status ?? '').toLowerCase()));
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {['all', 'pending', 'accepted', 'rejected', 'expired'].map((s) => (
              <button key={s} onClick={() => setContractFilter(s)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border ${contractFilter === s ? 'border-gold/40 bg-gold/10 text-champagne' : 'border-glass-border text-muted hover:text-white hover:bg-white/[0.04]'}`}>
            ...
            <div className="ml-auto flex items-center gap-2 bg-white/[0.04] border border-glass-border rounded-lg px-3 py-2 w-56">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search jockey, horse..." className="bg-transparent text-sm text-white placeholder:text-muted/60 outline-none w-full" />
            </div>
          </div>
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {visibleProposals.map((p, i) => {
                  ...
                  const { Icon } = cfg;
                  return (
                    <motion.div key={p.id ?? i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} className="glass-panel rounded-xl p-5 flex items-center gap-5 border border-glass-border hover:border-gold/30 hover:bg-gold/[0.04] transition-all group relative overflow-hidden">
                      ...
                      <div className="relative z-10 w-12 h-12 rounded-full bg-gradient-to-br from-blue-500/20 to-blue-900/20 border border-blue-500/20 ring-1 ring-gold/30 flex items-center justify-center font-serif font-bold text-blue-300 text-lg shrink-0">{(p.jockeyName ?? 'J')[0]}</div>
                      <div className="relative z-10 flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-xs text-muted">
                          ...
                          {p.startDate && (
                            <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-white/[0.04] border border-glass-border text-muted inline-flex items-center gap-1">
                              <Calendar size={9} className="text-gold/60" /> From: {formatDate(p.startDate)}
                            </span>
                          )}
                          {p.endDate && (
                            <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-white/[0.04] border border-glass-border text-muted inline-flex items-center gap-1">
                              <Calendar size={9} className="text-gold/60" /> Until: {formatDate(p.endDate)}
                            </span>
                          )}
                          {p.invitationExpiredAt && (
                            <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-white/[0.04] border border-glass-border text-red-400 inline-flex items-center gap-1">
                              <Clock size={9} className="text-red-400" /> Expires: {formatUtcDateTime(p.invitationExpiredAt)}
                            </span>
                          )}
                      ...
                      <div className="relative z-10 flex items-center gap-2 shrink-0">
                        ...
                        {String(p.status ?? '').toLowerCase() === 'pending' && (
                          <button onClick={() => handleCancelInvite(p.id)} className="px-3 py-1 rounded-full text-[11px] font-bold text-red-400 border border-red-500/20 bg-red-500/10 hover:bg-red-500/20 transition-colors">
                            Cancel invitation
                          </button>
                        )}
                      </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-md border border-gold/20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={form.tournamentId} disabled={isPrefilled} onChange={(e) => handleTournamentSelect(e.target.value)} className={`${INPUT} disabled:opacity-50 disabled:cursor-not-allowed`}>
              ...
              <div>
                ...
                <select value={form.horseId} disabled={isPrefilled || !form.tournamentId} onChange={(e) => setForm((p) => ({ ...p, horseId: e.target.value }))} className={`${INPUT} disabled:opacity-50 disabled:cursor-not-allowed`}>
                  <option value="">{!form.tournamentId ? '-- Select Tournament First --' : '-- Select Horse --'}</option>
                  {filteredHorsesForSelect.map((h) => (
                    <option key={h.id} value={h.id}>
                      {h.name}
                    </option>
                  ))}
                </select>
                {form.tournamentId && filteredHorsesForSelect.length === 0 && <p className="text-[10px] text-yellow-400 mt-1">No horses have passed vet inspection for this tournament.</p>}
                {horseBusyError && <p className="text-[11px] text-red-400 mt-1.5 font-medium">{horseBusyError}</p>}
              </div>

              <div>
                <label className={LABEL}>Select Jockey *</label>
                <select value={form.jockeyId} disabled={!form.tournamentId || loadingBusyJockeys} onChange={(e) => setForm((p) => ({ ...p, jockeyId: e.target.value }))} className={`${INPUT} disabled:opacity-50 disabled:cursor-not-allowed`}>
                  <option value="">{!form.tournamentId ? '-- Select Tournament First --' : loadingBusyJockeys ? '-- Loading available jockeys... --' : '-- Select Jockey --'}</option>
                ...
                {form.tournamentId && !loadingBusyJockeys && filteredJockeysForSelect.length === 0 && <p className="text-[10px] text-yellow-400 mt-1">All jockeys are already contracted or registered for this tournament.</p>}
                {jockeyBusyError && <p className="text-[11px] text-red-400 mt-1.5 font-medium">{jockeyBusyError}</p>}
              </div>
              ...
              <div>
                ...
                <input type="number" min="0.1" step="any" value={form.expirationHours} onChange={(e) => setForm((p) => ({ ...p, expirationHours: e.target.value }))} placeholder="Enter deadline in hours (e.g. 24)" className={INPUT} />
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={closeInvite} className="flex-1 py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                Cancel
              </button>
