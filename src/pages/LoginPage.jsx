...
function HorseArt() {
  ...
  return <img src="/images/horse-lineart.png" alt="Racehorse and jockey" className="w-full h-full object-contain drop-shadow-[0_0_18px_rgba(212,175,55,0.35)] translate-x-[18%]" onError={() => setUseFallback(true)} />;
}

function HorseSVG() {
  // Line-art ngựa phi nước đại + jockey cúi người (nét vàng, phong cách khắc mảnh)
  return (
    <svg viewBox="0 0 640 400" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full drop-shadow-[0_0_18px_rgba(212,175,55,0.35)]">
...
export function LoginPage() {
  ...
  const fieldError = (name) => (formik.touched[name] && formik.errors[name] ? formik.errors[name] : '');
    ...
    <GoogleOAuthProvider clientId={googleClientId}>
      <div
        ...
        style={{
          ...
          backgroundImage: 'radial-gradient(ellipse at 0% 100%, rgba(212,175,55,0.15) 0%, transparent 50%), radial-gradient(ellipse at 100% 0%, rgba(212,175,55,0.1) 0%, transparent 40%)',
      ...
      >
        ...
        <div className="absolute top-0 left-0 w-full px-12 py-8 z-20 bg-transparent">
          <Link to="/" className="tracking-widest font-semibold cursor-pointer inline-block" style={{ fontFamily: '"Playfair Display", serif', color: '#d4af37', fontSize: '26px' }}>
            EQUESTRIA
          </Link>
        </div>
        ...
        <main className="flex-grow flex items-center justify-center relative z-10 px-6 py-12 lg:px-16 w-full max-w-[1600px] mx-auto">
          {/* Vertical divider */}
          <div className="absolute hidden lg:flex flex-col items-center pointer-events-none z-0" style={{ left: '50%', transform: 'translateX(-50%)', top: '10%', bottom: '10%' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-center w-full">
            {/* Left: Branding */}
            <motion.section className="hidden lg:flex flex-col items-center justify-center text-center px-8" initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
              ...
              <div className="mb-10">
                <h1 className="italic font-medium mb-6 leading-tight" style={{ fontFamily: '"Playfair Display", serif', color: '#e9c46a', fontSize: '52px', lineHeight: '1.15' }}>
                  "Where Champions
                  <br />
                  Are Made."
                </h1>
              ...
              <div className="flex gap-3">
                {['5 Races', 'Real-time', 'Secure'].map((s) => (
                  <span key={s} className="px-5 py-2 rounded-full font-medium tracking-wide auth-badge" style={{ border: '1px solid rgba(212,175,55,0.2)', color: '#d4af37', background: 'rgba(30,41,59,0.3)', fontSize: '13px' }}>
                    {s}
                  </span>
                ))}
              </div>
            ...
            <section className="w-full flex justify-center">
              ...
              >
                ...
                >
                  ...
                  <div className="absolute top-3 right-3 pointer-events-none scale-x-[-1]">
                    <CornerOrnament />
                  </div>
                  <div className="absolute bottom-3 left-3 pointer-events-none scale-y-[-1]">
                    <CornerOrnament />
                  </div>
                  <div className="absolute bottom-3 right-3 pointer-events-none scale-x-[-1] scale-y-[-1]">
                    <CornerOrnament />
                  </div>
                  ...
                  <motion.div variants={fadeUp} className="text-center mb-8 relative z-10">
                    ...
                    <h2 className="text-2xl tracking-widest mb-2" style={{ fontFamily: '"Playfair Display", serif', color: '#d4af37' }}>
                      EQUESTRIA
                    </h2>
                  ...
                  <div className="space-y-5 relative z-10">
                    ...
                    <motion.div variants={fadeUp}>
                      ...
                      <input className="w-full rounded-md px-4 py-3 text-sm outline-none transition-all duration-300 border border-slate-400/20 focus:border-[#d4af37]" style={{ background: 'rgba(15,23,42,0.6)', color: '#e2e8f0' }} type="email" placeholder="email@example.com" name="email" value={formik.values.email} onChange={formik.handleChange} onKeyDown={(e) => e.key === 'Enter' && formik.handleSubmit()} onBlur={formik.handleBlur} />
                    ...
                    <motion.div variants={fadeUp}>
                      ...
                      <div className="relative">
                        <input className="w-full rounded-md px-4 py-3 pr-10 text-sm outline-none transition-all duration-300 border border-slate-400/20 focus:border-[#d4af37]" style={{ background: 'rgba(15,23,42,0.6)', color: '#e2e8f0' }} type={showPassword ? 'text' : 'password'} placeholder="••••••••" name="password" value={formik.values.password} onChange={formik.handleChange} onKeyDown={(e) => e.key === 'Enter' && formik.handleSubmit()} onBlur={formik.handleBlur} />

                        <button className="absolute inset-y-0 right-0 flex items-center pr-3" style={{ color: '#94a3b8' }} type="button" onClick={() => setShowPassword(!showPassword)}>
                    ...
                    {/* Error message */}
                    {error && (
                      <motion.div variants={fadeUp} className="rounded-md px-4 py-3 text-sm" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.35)', color: '#fca5a5' }}>
                        {error}
                      </motion.div>
                    )}
                    ...
                    {/* Submit */}
                    <motion.div variants={fadeUp}>
                      <button className="w-full font-bold text-sm tracking-wider uppercase py-3.5 rounded-md flex items-center justify-center gap-2 transition-all duration-300 hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] disabled:opacity-60 disabled:cursor-not-allowed" style={{ background: 'linear-gradient(135deg,#e9c46a 0%,#d4af37 50%,#aa8c2c 100%)', color: '#0b101e' }} type="button" onClick={() => formik.handleSubmit()} disabled={loading}>
                    ...
                    <motion.div variants={fadeUp} className="flex flex-col items-center gap-4">
                      ...
                      <div className={`w-full flex justify-center ${loading ? 'pointer-events-none opacity-50' : ''}`} style={{ minHeight: '44px' }}>
                        <GoogleLogin onSuccess={handleGoogleSuccess} onError={handleGoogleError} theme="filled_blue" shape="rectangular" size="large" width={368} />
                      </div>
                    </motion.div>
                    ...
                    <motion.div variants={fadeUp} className="text-center text-xs" style={{ color: '#94a3b8' }}>
                      ...
                      <Link to="/register" className="font-medium transition-colors" style={{ color: '#d4af37' }}>
                        Sign up here
                      </Link>
                    </motion.div>
                  </div>
