...
import { Shield, Trophy, Users, Flag, Stethoscope, Eye, ArrowRight, Wallet, Bell } from 'lucide-react';
    ...
    <div className="min-h-screen bg-navy text-body font-sans selection:bg-gold/30">
      ...
      <main className="max-w-6xl mx-auto px-6 pt-32 pb-20 space-y-20">
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="landing-section text-center">
          ...
          <p className="text-sm text-muted max-w-2xl mx-auto leading-relaxed">Equestria is a horse racing tournament management system. It keeps every party of a race meeting — organizers, owners, jockeys, referees, veterinarians and spectators — working on the same schedule, the same entry list and the same official results, instead of scattered spreadsheets and phone calls.</p>
        </motion.section>
        ...
        <section className="landing-section">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {ROLES.map((r, i) => (
              <motion.div key={r.title} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="glass-panel rounded-2xl p-6 border border-glass-border hover:border-gold/25 transition-all h-full">
        ...
        <section className="landing-section">
          ...
          <p className="text-sm text-muted mb-8">The system enforces this order, so a race cannot start with an unapproved or unfit horse.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {FLOW.map((s, i) => (
              <motion.div key={s.step} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="glass-panel rounded-2xl p-6 border border-glass-border relative overflow-hidden h-full">
        ...
        <section className="landing-section grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            ...
            <motion.div key={f.title} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="glass-panel rounded-2xl p-6 border border-glass-border h-full">
        ...
        <section className="landing-section glass-panel rounded-2xl p-10 text-center border border-gold/20 relative overflow-hidden">
          ...
          <p className="text-sm text-muted mb-6 max-w-md mx-auto">Create an account to follow live races, or sign in if your stable is already registered.</p>
          <div className="flex items-center justify-center gap-3 flex-wrap">
            ...
            <button onClick={() => navigate('/login')} className="px-7 py-3 rounded-lg text-xs font-bold border border-glass-border text-body hover:text-white hover:bg-white/5 transition-colors">
              Sign in
            </button>
          </div>
        </section>
