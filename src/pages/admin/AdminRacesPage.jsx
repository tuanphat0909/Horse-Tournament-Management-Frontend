...
function formatToDatetimeLocal(v) {
  if (!v) return '';
  const d = new Date(v);
...
export function AdminRacesPage() {
  ...
  const [openTournaments, setOpenTournaments] = useState(new Set());
  ...
  const [generatingForTournament, setGeneratingForTournament] = useState(null);
  ...
  function handleEditRaceClick(race) {
    setEditingRace(race);
    setEditRaceForm({
      name: race.name || '',
  ...
  async function handleUpdateRace() {
    if (!editRaceForm.name.trim() || !editRaceForm.raceDate || !editRaceForm.distanceMeter || !editRaceForm.maxLanes) {
      setEditRaceError('Please fill in all required fields.');
      return;
    }
    ...
    if (raceDateVal < new Date(now.getTime() - 5 * 60 * 1000)) {
      setEditRaceError('Race date cannot be in the past.');
      return;
    }
    ...
    try {
      ...
      showToast('Success', 'Race updated successfully!');
      setModal('none');
      setEditingRace(null);
      // Reload schedule data
      await loadAllData();
    } catch (err) {
      setEditRaceError(parseApiError(err));
    } finally {
  ...
  const [laneEntries, setLaneEntries] = useState([]); // entry assigned sẵn
  const [laneSel, setLaneSel] = useState({}); // laneNo -> registrationId
  const [laneLoading, setLaneLoading] = useState(false);
  const [laneSaving, setLaneSaving] = useState(false);
  const [laneMsg, setLaneMsg] = useState(null);
  const [swapFromEntry, setSwapFromEntry] = useState(null);
  const [swapToLane, setSwapToLane] = useState(0);
  ...
  function setR(field, v) {
    setRaceForm((p) => ({ ...p, [field]: v }));
  }
  function setF(field, v) {
    setRefForm((p) => ({ ...p, [field]: v }));
  }
  ...
  async function loadAllData() {
    ...
    setFetchError('');
    try {
      const [tRes, rRes, regRes, refRes] = await Promise.all([getTournaments(), getRaceSchedule(), getRegistrations(), getReferees()]);
  ...
  async function handleGenerateRaces(tournamentId) {
  ...
  async function handleGenerateFinal(tournamentId) {
    setGeneratingForTournament(tournamentId);
    try {
  ...
  async function handleCreateRace() {
    ...
    try {
      ...
      showToast('Success', newId != null ? `Race created "${raceForm.name}" successful! (ID = ${newId})` : `Race created "${raceForm.name}" successful!`);
      closeModal();
      await loadAllData();
      window.dispatchEvent(new Event('tournament-readiness-changed'));
    } catch (err) {
      setRaceError(parseApiError(err));
    } finally {
  ...
  async function openLanes(raceId) {
    setModal('lanes');
    setLaneMsg(null);
    setLaneSel({});
  ...
  async function selectLaneRace(rid) {
    setLaneRaceId(rid);
    setLaneSel({});
    setLaneMsg(null);
    setLaneEntries([]);
  ...
  const eligibleRegs = useMemo(() => {
    if (!laneRace) return [];
    const takenRegIds = new Set(laneEntries.map((e) => e.registrationId));
    const takenHorseIds = new Set(laneEntries.map((e) => e.horseId));
    return registrationsList.filter((r) => {
      const regId = r.registrationId ?? r.id;
      return Number(r.tournamentId ?? r.TournamentId) === Number(laneRace.tournamentId) && (r.status ?? '').toLowerCase() === 'approved' && (r.healthStatus ?? 'Healthy').toLowerCase() === 'healthy' && !takenRegIds.has(regId) && !takenHorseIds.has(r.horseId);
    });
  ...
  async function handleSaveLanes() {
    ...
    for (const [laneNo, regId] of picks) {
      try {
        ...
        errors.push(`Lane ${laneNo}: ${parseApiError(err)}`);
      }
    }
    setLaneSaving(false);
    if (errors.length === 0) {
  ...
  async function openRefereeModal(raceId) {
  ...
  async function handleAssignReferee() {
    ...
    try {
      ...
      setRefError(parseApiError(err));
    } finally {
      setRefLoading(false);
    }
  }

  async function handleViewReferees(raceId) {
    if (!raceId) {
      setReferees([]);
      return;
    }
    ...
    try {
      const data = await getRaceReferees(Number(raceId));
  ...
  async function handleRemoveReferee(raceId, refereeId) {
    try {
  ...
  async function handleWithdrawEntry(raceEntryId, horseName) {
  ...
  const groupedTournaments = closedTournamentsList.map((t) => {
    const normalizedTournamentStatus = String(t.status ?? '')
      .trim()
      .toLowerCase();
    ...
    const finalDone = finalRaces.length > 0 && finalRaces.every((r) => r.status === 'Finished' || r.status === 'Completed');
    ...
    const waitingLabel = hasPrefinalRaces && !prefinalFinished ? 'Pending completed Pre' : '';
    ...
    const hasAnyRaces = rounds.some((r) => r.races.length > 0);
    ...
    const scheduledRaces = rounds
      .flatMap((r) => r.races)
      .filter((r) => r.status === 'Scheduled' && r.raceDate)
    ...
    const hasPastScheduledRace = scheduledRaces.some((r) => new Date(r.raceDate).getTime() <= now.getTime());

    const scheduledPreRaces = (prefinalRound?.races ?? []).filter((r) => r.status === 'Scheduled' && r.raceDate).sort((a, b) => new Date(a.raceDate).getTime() - new Date(b.raceDate).getTime());
    const preRoundStartDate = scheduledPreRaces[0]?.raceDate ?? null;
    const hasPastScheduledPreRace = scheduledPreRaces.some((r) => new Date(r.raceDate).getTime() <= now.getTime());
    const hasPreRaceProgress = Boolean(prefinalRound?.races?.some((r) => r.status === 'Live' || r.status === 'InProgress' || r.status === 'Finished' || r.status === 'Completed'));

    const isPreStarted = Boolean(hasPreRaceProgress || (t.startDate && now >= new Date(t.startDate)));
    ...
    if (isTerminalTournament) {
      genHint = {
        ...
        detail: normalizedTournamentStatus === 'cancelled' ? 'This tournament is cancelled. Scheduling checks no longer apply.' : 'This tournament is completed. Historical horse counts and lane readiness no longer require action.',
      };
      ...
      genHint = {
        ...
        detail: qualifiedHorseCount < 12 ? `Only ${qualifiedHorseCount}/12 qualified horses. Race scheduling is blocked. Extend registration or cancel the tournament.` : `${qualifiedHorseCount}/48 qualified horses. Resolve the registration list before scheduling races.`,
      };
    } else if (!hasAnyRaces) {
      genHint = {
        tone: 'wait',
        label: tournamentStartPassed ? 'Tournament start blocked' : 'Lane assignment required',
        detail: tournamentStartPassed ? "The tournament start date has arrived, but no lanes are assigned. Click 'Auto Assign Lanes' before the tournament can become Active." : "Registration closed. Assign lanes before the tournament start date. Click 'Auto Assign Lanes' to generate the race schedule.",
      };
      ...
      if (hasPastScheduledPreRace && !hasPreRaceProgress) {
        ...
        genHint = {
          ...
          detail: isPreStarted ? 'Pending all Pre-round races completion to assign Finals.' : 'Pre-round races scheduled. Pending race start date and completion to assign Finals.',
        };
      }
    } else if (finalDone) {
      genHint = {
      ...
      genHint = {
        ...
        detail: hasPastScheduledRace ? 'The Final is still Scheduled. Start the race or update its status.' : 'This tournament goes directly to the Final. Waiting for the scheduled race start time.',
      };
    } else {
    ...
    return {
      ...
      status: String(t.status ?? '').toLowerCase() === 'active' ? (!hasAnyRaces ? 'PendingScheduling' : tournamentStartDate != null && now < tournamentStartDate ? 'Upcoming' : t.status) : t.status,
      rounds,
      canGeneratePre,
      canGenerateFinal,
      waitingLabel,
  ...
  const statusRank = (t) => STATUS_ORDER[getFilterType(t)] ?? 4;
  ...
  const activeRefereeOptions = refereeOptions.filter((ref) => !ref.status || String(ref.status).toLowerCase() === 'active');
  const visibleRefereeOptions = (activeRefereeOptions.length > 0 ? activeRefereeOptions : refereeOptions)
    ...
    .filter((ref) => !referees.some((a) => (a.refereeId ?? a.id) === ref.refereeId));
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          {loadingData && <LoadingSkeleton rows={4} h="h-24" />}

          {fetchError && <div className="px-6 py-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">Error loading data: {fetchError}</div>}
          ...
          {!loadingData && !fetchError && groupedTournaments.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap">
              {['all', 'pending_scheduling', 'upcoming', 'active', 'completed'].map((s) => (
                <button
                  key={s}
                  onClick={() => {
                    setStatusFilter(s);
                        ...
                        <div className={`flex flex-wrap items-center justify-between gap-4 ${isOpen ? 'border-b border-glass-border pb-4' : ''}`}>
                          <button onClick={() => toggleTournament(t.tournamentId)} className="text-left group flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-[10px] px-2 py-0.5 rounded-full border shrink-0 ${t.status === 'Active' || t.status === 'Ongoing' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : t.status === 'Upcoming' || t.status === 'PendingScheduling' ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' : t.status === 'Completed' || t.status === 'Finished' ? 'text-muted bg-white/5 border-glass-border' : 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20'}`}>{t.status ?? 'Pending'}</span>
                              {!isOpen && <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/4 border border-glass-border text-champagne shrink-0">{t.rounds.reduce((s, r) => s + r.races.length, 0)} races</span>}
                            </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {t.canGeneratePre && !t.regOpen && !t.regNotStarted && (
                              <div className="flex flex-col items-end">
                                <button
                                  onClick={() => {
                                    if (!isAutoAssignLanesDisabled) handleGenerateRaces(t.tournamentId);
                                  }}
                                  ...
                                  className={`px-4 py-2 text-xs font-bold rounded-lg transition-colors flex items-center gap-1.5 border ${isAutoAssignLanesDisabled ? 'bg-white/5 border-glass-border text-muted/40 cursor-not-allowed' : 'bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 border-blue-500/30'}`}
                                >
                            ...
                            {!t.isTerminalTournament && !t.hasAnyRaces && !t.regOpen && !t.regNotStarted && !t.hasValidHorseCount && (
                              <button disabled title="A tournament requires between 12 and 48 qualified horses before races can be scheduled." className="px-4 py-2 bg-red-500/10 text-red-400 border border-red-500/30 text-xs font-bold rounded-lg cursor-not-allowed flex items-center gap-1.5">
                            ...
                            {!t.isTerminalTournament && (t.regOpen || t.regNotStarted) && !t.hasAnyRaces && (
                              <button disabled title={t.regNotStarted ? `Registration opens at ${fmtDate(t.registrationStartDate)}, closes at ${fmtDate(t.registrationEndDate)}.` : `Registration closed at ${fmtDate(t.registrationEndDate)}. Pending registration expiry to assign lanes.`} className="px-4 py-2 bg-white/[0.04] text-muted border border-glass-border text-xs font-bold rounded-lg cursor-not-allowed flex items-center gap-1.5">
                            ...
                            {/* Final generation */}
                            {t.canGenerateFinal && (
                              <button onClick={() => handleGenerateFinal(t.tournamentId)} disabled={generatingForTournament === t.tournamentId} className="px-4 py-2 bg-gold/20 hover:bg-gold/30 text-gold border border-gold/30 text-xs font-bold rounded-lg transition-colors flex items-center gap-1.5 animate-pulse">
                                {generatingForTournament === t.tournamentId ? (
                                  <>
                                    <Loader size={12} className="animate-spin" />
                                    Creating...
                        ...
                        {isOpen && (
                          <div className="grid grid-cols-1 gap-6">
                            ...
                            <div className={`rounded-xl border px-4 py-3 flex items-start gap-3 ${HINT_TONE[t.genHint.tone]}`}>
                              {t.genHint.tone === 'wait' ? <AlertCircle size={16} className="shrink-0 mt-0.5" /> : t.genHint.tone === 'ready' ? <CheckCircle2 size={16} className="shrink-0 mt-0.5" /> : t.genHint.tone === 'progress' ? <Loader size={16} className="shrink-0 mt-0.5 animate-spin" /> : <Flag size={16} className="shrink-0 mt-0.5" />}
                            ...
                            {t.rounds.length === 0 && <div className="text-xs text-muted/50 italic py-2 text-center">No rounds. After registration closes, use 'Auto Assign Lanes' to generate Pre-round.</div>}
                            ...
                            {t.rounds.map((r) => (
                              <div key={r.roundId} className="space-y-3 bg-navy/20 p-4 rounded-xl border border-glass-border/40">
                                ...
                                {r.races.length === 0 ? (
                                  <div className="text-xs text-muted/50 italic py-4">No races scheduled. Use auto-assign or add manually.</div>
                                ) : (
                                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {r.races.map((race) => {
                                      return (
                                        <div key={race.raceId} className="bg-navy/40 border rounded-xl overflow-hidden transition-all duration-300 border-glass-border hover:border-white/10">
                                          <div className="p-4 space-y-3">
                                            <div className="flex items-start justify-between gap-2">
                                              <div>
                                                <h4 className="text-sm font-bold text-white flex items-center gap-1.5 flex-wrap">
                                                  🏁 {race.name}
                                                  {raceTypeBadge(race.name) && <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full border ${raceTypeBadge(race.name).cls}`}>{raceTypeBadge(race.name).label}</span>}
                                                </h4>
                                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${race.status === 'Finished' || race.status === 'Completed' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : race.status === 'Live' ? 'bg-red-500/10 text-red-400 border border-red-500/20 animate-pulse' : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'}`}>{race.status === 'Finished' || race.status === 'Completed' ? 'Completed' : race.status === 'Live' ? 'Active' : 'Scheduled'}</span>
                                              </div>
                                            ...
                                            <div className="flex items-center justify-end gap-2 pt-2 border-t border-glass-border/30">
                                              <div className="flex items-center gap-2">
                                                <button onClick={() => openDetail(race)} title="Details & 3D Lane Map" className="p-1.5 rounded-lg text-champagne hover:bg-gold/15 border border-transparent hover:border-gold/20 transition-colors">
                                                  <Eye size={13} />
                                                </button>
                                                <button onClick={() => openLanes(race.raceId)} title="Assign horse to lanes" className="p-1.5 rounded-lg text-blue-400 hover:bg-blue-500/10 border border-transparent hover:border-blue-500/20 transition-colors">
                                                  <ListOrdered size={13} />
                                                </button>
                                                <button onClick={() => openRefereeModal(race.raceId)} title="Assign referees" className="p-1.5 rounded-lg text-cyan-400 hover:bg-cyan-500/10 border border-transparent hover:border-cyan-500/20 transition-colors">
                                                  <UserCheck size={13} />
                                                </button>
                                                {race.status !== 'Finished' && race.status !== 'Completed' && race.status !== 'Live' && race.status !== 'InProgress' && race.status !== 'Cancelled' && (
                                                  <button onClick={() => handleEditRaceClick(race)} title="Edit race" className="p-1.5 rounded-lg text-amber-400 hover:bg-amber-500/10 border border-transparent hover:border-amber-500/20 transition-colors">
                                                    <Edit size={13} />
                                                  </button>
                                                )}
                                                {!isRaceLocked(race.status) && (
                                                  <button onClick={() => handleDeleteRace(race.raceId, race.name)} title="Delete race" className="p-1.5 rounded-lg text-red-400 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-colors">
                                                    <Trash2 size={13} />
                                                  </button>
                                                )}
                                              </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-gold/20 relative overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              {selectedTournamentId &&
                (selectedTournamentApprovedHorsesCount < 12 ? (
                  <div className="text-xs px-3 py-2.5 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 flex items-start gap-2 my-2 animate-pulse">
                    ...
                    <div>
                      <p className="font-bold mb-0.5">{'This tournament does not have enough 12 horses.'}</p>
                      <p className="opacity-80">{'Cannot schedule races. Please reopen registration or cancel the tournament.'}</p>
                    </div>
                  </div>
                ) : (
              ...
              <div>
                ...
                <select value={raceForm.roundId} onChange={(e) => setR('roundId', e.target.value)} disabled={!selectedTournamentId || selectedTournamentApprovedHorsesCount < 12} className={`${INPUT} bg-navy disabled:opacity-60 disabled:cursor-not-allowed`} style={{ colorScheme: 'dark' }}>
                  <option value="">{!selectedTournamentId ? '-- Select Tournament first --' : selectedTournamentApprovedHorsesCount < 12 ? '-- Not enough horses --' : '-- Select Round --'}</option>
                  {selectedTournamentApprovedHorsesCount >= 12 &&
              ...
              <div>
                ...
                <div className="relative">
                  <input type="datetime-local" value={raceForm.raceDate} onChange={(e) => setR('raceDate', e.target.value)} min={selectedRound ? formatToDatetimeLocal(selectedRound.startDate) : undefined} max={selectedRound ? formatToDatetimeLocal(selectedRound.endDate) : undefined} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleCreateRace} disabled={raceLoading || (selectedTournamentId !== '' && selectedTournamentApprovedHorsesCount < 12)} className="flex-1 btn-gold py-2.5 rounded-lg text-sm font-bold disabled:opacity-60 disabled:cursor-not-allowed">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-gold/20 relative overflow-hidden max-h-[90vh] overflow-y-auto text-left">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className={LABEL}>Race Date & Time *</label>
                <div className="relative">
                  <input type="datetime-local" value={editRaceForm.raceDate} onChange={(e) => setEditRaceForm((prev) => ({ ...prev, raceDate: e.target.value }))} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setModal('none')} className="flex-1 py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                Cancel
              </button>
              <button onClick={handleUpdateRace} disabled={editRaceLoading} className="flex-1 btn-gold py-2.5 rounded-lg text-sm font-bold disabled:opacity-60 disabled:cursor-not-allowed">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-2xl border border-blue-500/20 relative overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="relative flex items-center gap-3 mb-6">
              ...
              <button onClick={closeModal} className="p-1.5 rounded-lg text-muted hover:text-white hover:bg-white/10 transition-colors">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 px-4 py-3 rounded-lg bg-blue-500/5 border border-blue-500/20">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-muted truncate">
                    {laneRace?.tournamentName ?? ''}
                    {laneRace?.roundName ? ` • ${laneRace.roundName}` : ''} • {maxLanes} lanes • {fmtDate(laneRace?.raceDate)}
                  </div>
                </div>
              </div>

              {laneRaceId && laneRaceLocked && (
                <div className="text-[11px] rounded-lg px-3 py-2.5 leading-relaxed font-semibold" style={{ background: 'rgba(100,116,139,0.12)', border: '1px solid rgba(100,116,139,0.35)', color: '#475569' }}>
              ...
              {laneRaceId && !laneRaceLocked && (
                <div className="text-[11px] text-champagne/80 bg-gold/5 border border-gold/15 rounded-lg px-3 py-2 leading-relaxed">
                  Select a horse for each empty lane and click <b>"Save Selected Lanes"</b>. Only displays horses with <b>approved status (Approved) + Healthy status</b>, not yet assigned — horses selected in other lanes will be hidden. Jockey is automatically assigned based on accepted jockey contract.
                </div>
              )}
              ...
              {laneRaceId &&
                !laneRaceLocked &&
                (() => {
                  const unhealthy = registrationsList.filter((r) => Number(r.tournamentId ?? r.TournamentId) === Number(laneRace?.tournamentId) && (r.status ?? '').toLowerCase() === 'approved' && (r.healthStatus ?? 'Healthy').toLowerCase() !== 'healthy');
                  ...
                  if (totalApproved <= 12) {
                    return (
                      <div className="rounded-lg bg-emerald-500/8 border border-emerald-500/25 px-3 py-2.5 text-[11px] text-emerald-400 space-y-0.5">
                        ...
                        <div>
                          <b>{totalApproved}</b> eligible horses ≤ 12 → all <b>{totalApproved}</b> horses go straight to this finals race, no qualifiers needed.
                        </div>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div className="rounded-lg bg-orange-500/10 border border-orange-500/30 px-3 py-2.5 space-y-2 text-[11px]">
                      ...
                      <div className="text-orange-400">
                        <b>{totalApproved}</b> eligible horses &gt; 12 → split evenly into <b>{heats.length}</b> qualifier races:
                      </div>
                      <div className="flex flex-wrap gap-1.5">
                        {heats.map((size, i) => (
                          <span key={i} className="px-2.5 py-1 rounded-lg bg-orange-500/15 border border-orange-500/25 text-orange-400 font-bold">
                            Qualifier {i + 1}: {size} horse
                          </span>
                        ))}
                      ...
                      <div className="text-orange-400/80">
                        After qualifiers → choose <b className="text-orange-400">12 horses with best race times</b> → finals race. Please create <b>{heats.length}</b> qualifier races + <b>1</b> finals race in this round.
                      </div>
                    </div>
                  );
                })()}
              ...
              {laneLoading ? (
                ...
                <div className="space-y-2">
                  {Array.from({ length: maxLanes }, (_, idx) => idx + 1).map((laneNo) => {
                    const existing = laneEntries.find((e) => e.laneNo === laneNo);
                    if (existing) {
                      ...
                      return (
                        <div key={laneNo} className="space-y-1">
                          <div className={`flex items-center gap-3 px-4 py-2.5 rounded-lg border ${isUnhealthy ? 'bg-red-500/8 border-red-500/30' : 'bg-emerald-500/5 border-emerald-500/20'}`}>
                            ...
                            {isUnhealthy ? <AlertCircle size={14} className="text-red-400 shrink-0" /> : <CheckCircle2 size={14} className="text-emerald-400 shrink-0" />}
                            ...
                            {isUnhealthy && <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/20 border border-red-500/30 text-red-300 font-bold shrink-0">{existing.healthStatus}</span>}
                            ...
                            {laneRaceLocked ? null : isUnhealthy ? (
                              <button onClick={() => handleWithdrawEntry(existing.raceEntryId, existing.horseName ?? `Horse #${existing.horseId}`)} disabled={withdrawingEntryId === existing.raceEntryId} className="text-[10px] px-2 py-1 rounded border border-red-500/40 bg-red-500/15 text-red-400 hover:bg-red-500/25 shrink-0 font-bold disabled:opacity-50">
                                {withdrawingEntryId === existing.raceEntryId ? '...' : 'Remove'}
                              </button>
                            ) : (
                              <button
                                onClick={() => {
                                  setSwapFromEntry(existing);
                                  setSwapToLane(0);
                                }}
                          ...
                          {isSwapping && (
                            <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-500/5 border border-blue-500/25">
                              ...
                              <select value={swapToLane} onChange={(e) => setSwapToLane(Number(e.target.value))} className="flex-1 bg-navy/50 border border-glass-border rounded-lg px-2 py-1 text-xs text-white outline-none focus:border-blue-400/40" style={{ colorScheme: 'dark' }}>
                                <option value={0}>— Select Target Lane —</option>
                                {Array.from({ length: maxLanes }, (_, i) => i + 1)
                                  .filter((l) => l !== laneNo)
                                  .map((l) => {
                                    const occ = laneEntries.find((e) => e.laneNo === l);
                                    return (
                                      <option key={l} value={l}>
                                        Lane {l}
                                        {occ ? ` ↔ ${occ.horseName ?? 'horse'}` : ' (empty)'}
                                      </option>
                                    );
                                  })}
                              </select>
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  const fromLane = laneNo;
                                  const toLane = swapToLane;
                                  const toEntry = laneEntries.find((e) => e.laneNo === toLane);
                                  setLaneEntries((prev) =>
                                    prev.map((e) => {
                                  ...
                                  setSwapFromEntry(null);
                                  setSwapToLane(0);
                                  showToast('Lane Swapped [MOCK]', toEntry ? `Swap Lane ${fromLane} ↔ Lane ${toLane}: ${existing.horseName} ↔ ${toEntry.horseName}` : `Move ${existing.horseName} from Lane ${fromLane} → Lane ${toLane}`);
                                }}
                                className="text-[11px] px-3 py-1 rounded-lg bg-blue-500/20 border border-blue-500/30 text-blue-400 hover:bg-blue-500/30 disabled:opacity-40 shrink-0 font-bold"
                              >
                                Confirm
                              </button>
                              <button
                                ...
                                className="text-[11px] px-2 py-1 rounded-lg border border-glass-border text-muted hover:text-white shrink-0"
                              >
                                Cancel
                              </button>
                            </div>
                    ...
                    const usedElsewhere = new Set(
                      Object.entries(laneSel)
                        .filter(([l]) => Number(l) !== laneNo)
                        .map(([, v]) => v)
                        .filter(Boolean)
                    );
                    ...
                    if (laneRaceLocked) {
                      ...
                      return (
                        <div key={laneNo} className="flex items-center gap-3 px-4 py-2 rounded-lg bg-white/2 border border-glass-border">
                          ...
                          <span className="text-xs italic" style={{ color: 'rgba(100,116,139,0.8)' }}>
                            Not used in this race
                          </span>
                        </div>
                      );
                    }
                    return (
                      <div key={laneNo} className="flex items-center gap-3 px-4 py-2 rounded-lg bg-white/2 border border-glass-border">
                        <span className="w-14 shrink-0 text-xs font-bold text-muted">Lane {laneNo}</span>
                        <select value={laneSel[laneNo] ?? ''} onChange={(e) => setLaneSel((p) => ({ ...p, [laneNo]: e.target.value }))} className="flex-1 bg-navy/50 border border-glass-border rounded-lg px-3 py-1.5 text-sm text-white outline-none focus:border-blue-400/40" style={{ colorScheme: 'dark' }}>
                          <option value="">— Leave empty —</option>
                          {opts.map((r, i) => {
                            const regId = r.registrationId ?? r.id;
                            return (
                              <option key={regId ?? i} value={regId}>
                  ...
                  {laneEntries.some((e) => (e.laneNo ?? 0) > maxLanes) && (
                    <div className="rounded-lg bg-red-500/10 border border-red-500/30 px-3 py-2 text-[11px] text-red-400">
                      ⚠ There are {laneEntries.filter((e) => (e.laneNo ?? 0) > maxLanes).length} horses assigned <b>exceeding {maxLanes} lanes</b> (invalid data from backend):{' '}
                      {laneEntries
                        .filter((e) => (e.laneNo ?? 0) > maxLanes)
                        .map((e) => `Lane ${e.laneNo} • ${e.horseName ?? 'Horse #' + e.horseId}`)
                        .join(', ')}
                    </div>
                  )}
            ...
            <div className="flex gap-3 mt-6">
              ...
              {!laneRaceLocked && (
                <button onClick={handleSaveLanes} disabled={laneSaving || !laneRaceId} className="flex-1 py-2.5 rounded-lg bg-blue-500/20 text-blue-400 border border-blue-500/30 hover:bg-blue-500/30 text-sm font-bold disabled:opacity-60 disabled:cursor-not-allowed transition-colors">
            ...
            <div className="relative flex items-center gap-3 mb-6">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-muted">
                  {detailRace.tournamentName ?? '—'}
                  {detailRace.roundName ? ` • ${detailRace.roundName}` : detailRace.roundNumber != null ? ` • Round ${detailRace.roundNumber}` : ''}
                </p>
              </div>
                ...
                <div className="mb-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {(() => {
                      ...
                      const valid = detailEntries.filter((e) => (e.laneNo ?? 0) <= maxL).length;
                      ...
                      return (
                        <>
                          ...
                          {over > 0 && <span className="text-[10px] px-2 py-0.5 rounded-full bg-red-500/15 border border-red-500/30 text-red-400 font-bold">+{over} exceeded lanes (data error)</span>}
                        </>
                      );
                    })()}
                  </div>
                  ...
                  {detailEntries.length > 0 && (
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                      {[...detailEntries]
                        .sort((a, b) => (a.laneNo ?? 0) - (b.laneNo ?? 0))
                        .map((e, i) => {
                            ...
                            <div key={i} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs ${isOver ? 'bg-red-500/10 border border-red-500/30' : isUnhealthy ? 'bg-red-500/8 border border-red-500/25' : 'bg-white/2 border border-glass-border'}`}>
                              ...
                              {isUnhealthy && !isOver && (
                                <>
                                  ...
                                  <button onClick={() => handleWithdrawEntry(e.raceEntryId, e.horseName ?? `Horse #${e.horseId}`)} disabled={withdrawingEntryId === e.raceEntryId} className="text-[10px] px-1.5 py-0.5 rounded border border-red-500/40 bg-red-500/15 text-red-400 hover:bg-red-500/25 shrink-0 font-bold disabled:opacity-50">
                                    {withdrawingEntryId === e.raceEntryId ? '...' : 'Remove'}
                                  </button>
                                </>
                              )}
                ...
                <div>
                  ...
                  {detailRefs.length === 0 ? (
                    <div className="text-xs text-muted/60 italic px-1">
                      No referee assigned — use <UserCheck size={11} className="inline" /> button in list to assign.
                    </div>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {/* Màu cyan đậm cố định: cyan-300 quá nhạt nên chìm trên nền sáng */}
                      {detailRefs.map((r, i) => (
                        <span key={i} className="text-xs font-semibold px-3 py-1.5 rounded-full" style={{ background: 'rgba(6,182,212,0.14)', border: '1px solid rgba(6,182,212,0.45)', color: '#0E7490' }}>
      ...
      {modal === 'referee' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-cyan-500/20 relative overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 px-4 py-3 rounded-lg bg-cyan-500/5 border border-cyan-500/20">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-muted truncate">
                    {refRace?.tournamentName ?? ''} • {fmtDate(refRace?.raceDate)}
                  </div>
                </div>
              </div>

              {refRaceLocked ? (
                <div className="rounded-lg bg-white/[0.03] border border-glass-border px-4 py-2.5 text-[11px] text-muted">This race has finished — referee assignments are read-only.</div>
              ) : (
                <div>
              ...
              {refForm.raceId && (
                <div className="pt-2 border-t border-glass-border">
                  <div className="text-xs font-bold text-muted uppercase tracking-wider mb-3">Referees of the race {refViewLoading ? '(loading...)' : `(${referees.length})`}</div>
                  {referees.length > 0 ? (
                    <div className="space-y-2">
                      {referees.map((r, i) => {
                        ...
                        return (
                          <div key={id ?? i} className="flex items-center justify-between gap-3 px-4 py-2.5 rounded-lg bg-white/[0.02] border border-glass-border">
                            ...
                            {!refRaceLocked && (
                              <button onClick={() => handleRemoveReferee(refForm.raceId, id)} className="p-1.5 rounded-lg text-red-400 hover:bg-red-500/10 transition-colors" title="Remove referee">
                                <Trash2 size={14} />
                              </button>
                            )}
                          </div>
                    ...
                    !refViewLoading && <div className="text-center py-3 text-muted text-xs">No referees assigned</div>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="mt-6">
              <button onClick={closeModal} className="w-full py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                Close
              </button>
            </div>
          </motion.div>
