...
function getTournamentCustomStatus(tour) {
...
export function AdminTournamentsPage() {
  ...
  const [cancelWarningTournament, setCancelWarningTournament] = useState(null);
  ...
  async function handleUpdate() {
    ...
    if (regEndVal <= regStartVal) {
      setEditError('Registration end date must be after registration start date.');
      return;
    }
    ...
    if (endVal <= startVal) {
      setEditError('Tournament end date must be after tournament start date.');
      return;
    }
  ...
  async function handleExtendRegistration() {
    ...
    try {
      ...
      showToast('Success', 'Registration extended successfully!');
  ...
  function handleCloseRegistrationClick(tour) {
  ...
  async function handleCancelTournament() {
    ...
    const confirmMessage = isSecondClose ? 'Are you sure you want to cancel this tournament because there are not enough registered horses after 2 registration cycles?' : 'Are you sure you want to cancel this tournament?';
    ...
    try {
      const reason = isSecondClose ? 'Not enough registered horses after 2 registration cycles.' : 'Tournament starts in less than 24 hours.';
  ...
  async function loadTournaments() {
    ...
    try {
      const [data, raceData] = await Promise.all([getTournaments(), getRaceSchedule().catch(() => ({ result: [] }))]);
  ...
  async function handleCreate() {
    if (!form.name.trim() || !form.registrationStartDate || !form.registrationEndDate || !form.startDate || !form.endDate || !form.firstPrize || !form.secondPrize || !form.thirdPrize) {
    ...
    if (endVal <= startVal) {
      setError('Tournament end date must be after tournament start date.');
      return;
    }
    ...
    try {
      ...
      showToast('Success', newId != null ? `${'Tournament created successfully!'} ID = ${newId}. ${'Tournament is now in Pending Registration status.'}` : 'Tournament created successfully!');
      setForm(INIT_FORM);
      setShowModal(false);
      loadTournaments();
    } catch (err) {
  ...
  async function handleCloseRegistration(tournamentId) {
    setGeneratingForTournament(tournamentId);
    try {
      await closeTournamentRegistration(tournamentId);
      showToast('Success', 'Registration deadline closed successfully!');
      await loadTournaments();
    } catch (err) {
  ...
  async function handleGenerateFinal(tournamentId) {
    ...
    try {
      ...
      showToast('Success', 'Final bracket (Top 12) auto-assigned successfully.');
      await loadTournaments();
    } catch (err) {
      const errorMsg = parseApiError(err);
      showToast('Failed', errorMsg, 'error');
  ...
  function getTournamentRaceState(tour) {
    ...
    const preRound = rounds.find((r) => r.roundNumber === 1);
    ...
    const isSuspended = tour.status?.toLowerCase() === 'registration suspended' || (tour.status?.toLowerCase() === 'registration open' && regEnded && cancelCount === 0);
    ...
    if (isSuspended) {
      ...
      const isPreStarted = Boolean(preRound?.races?.some((r) => r.status === 'Live' || r.status === 'InProgress' || r.status === 'Finished' || r.status === 'Completed') || (tour.startDate && now >= new Date(tour.startDate)));
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 relative min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {['all', 'upcoming_registration', 'registration_open', 'registration_closed', 'scheduled', 'racing', 'completed', 'cancelled'].map((s) => {
              const labelMap = {
                all: 'All',
              ...
              return (
                <button key={s} onClick={() => setFilter(s)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border ${filter === s ? 'border-gold/40 bg-gold/10 text-champagne' : 'border-glass-border text-muted hover:text-white hover:bg-white/[0.04]'}`}>
                  {labelMap[s]}
                  <span className="ml-2 text-[11px] font-bold text-current opacity-60">{statsCounts[s] ?? 0}</span>
                </button>
              );
            })}
            <div className="ml-auto flex items-center gap-2 bg-white/[0.04] border border-glass-border rounded-lg px-3 py-2 w-64">
              <Search size={14} className="text-muted shrink-0" />
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={'Search tournaments...'} className="bg-transparent text-sm text-white placeholder:text-muted/60 outline-none focus:outline-none focus:ring-0 border-0 w-full" style={{ boxShadow: 'none' }} />
            </div>
            <div className="flex items-center gap-2">
              <ArrowUpDown size={14} className="text-muted" />
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-navy/50 border border-glass-border rounded-lg px-3 py-2 text-xs text-white outline-none focus:border-gold/40 transition-colors" style={{ colorScheme: 'dark' }}>
            ...
            <div className="overflow-y-auto pr-1.5 -mr-1.5 scrollbar-thin" style={{ maxHeight: 'calc(100vh - 330px)' }}>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {sortedTournaments.map((tour, i) => {
                  ...
                  return (
                    <motion.div key={tour.tournamentId ?? i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-panel rounded-2xl p-5 border border-glass-border hover:border-gold/25 transition-all group relative overflow-hidden text-left h-full flex flex-col">
                      <div className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-gold/40 to-transparent pointer-events-none" />
                      ...
                      <p className="text-xs text-muted/80 line-clamp-2 min-h-[32px] mb-3">{tour.description || 'No detailed description available.'}</p>
                      <div className="space-y-1.5 text-xs text-muted pt-3 border-t border-glass-border/40">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-gold font-bold">{raceState.statusLabel}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>{'Races created:'}</span>
                        ...
                        {(customStatus === 'Registration Closed' || customStatus === 'Scheduled' || customStatus === 'Racing' || customStatus === 'Completed') && (
                          <div className="flex justify-between">
                            <span>{'Qualified Horses:'}</span>
                            <span className={`font-bold ${(tour.qualifiedRegistration ?? 0) >= 12 ? 'text-emerald-400' : 'text-red-400'}`}>{tour.qualifiedRegistration ?? 0} / 12</span>
                          </div>
                        )}
                        <div className="flex flex-col gap-1 pt-2.5 mt-2 border-t border-glass-border/30">
                          <span className="font-bold text-white text-[11px] uppercase tracking-wider">{'Prizes:'}</span>
                          ...
                          <div className="min-h-[46px] flex flex-col justify-center">
                            {tour.prizes && tour.prizes.length > 0 ? (
                              ...
                              <span className="text-red-400 font-semibold italic text-[11px]">{'Prizes not configured yet'}</span>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-auto pt-4 flex flex-wrap gap-2 w-full">
                        {customStatus === 'Registration Open' && (
                          <div className="flex gap-2 w-full">
                            <button onClick={() => handleCloseRegistrationClick(tour)} disabled={isGenerating} className="flex-1 px-3 py-2 rounded-lg text-xs font-bold text-red-400 border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 disabled:opacity-60 transition-colors flex items-center justify-center gap-1.5">
                              {isGenerating ? <Loader size={13} className="animate-spin" /> : <Clock size={13} />}
                              {isGenerating ? 'Closing...' : 'Close Registration'}
                            </button>
                            <button onClick={() => setCancelWarningTournament(tour)} disabled={isGenerating} className="px-3 py-2 rounded-lg text-xs font-bold text-red-500 border border-red-500/50 bg-red-500/10 hover:bg-red-500/20 disabled:opacity-60 transition-colors flex items-center justify-center gap-1.5">
                              {'Cancel'}
                            </button>
                          </div>
                        )}
                        ...
                        {customStatus === 'Registration Closed' && (
                          <div className="flex flex-col gap-2.5 w-full">
                            {(tour.qualifiedRegistration ?? 0) >= 12 ? (
                              <div className="flex gap-2 w-full">
                                <button onClick={() => navigate('/admin/races', { state: { openTournamentId: tour.tournamentId } })} className="flex-1 px-3 py-2 rounded-lg text-xs font-bold text-blue-400 border border-blue-500/30 bg-blue-500/10 hover:bg-blue-500/20 transition-colors flex items-center justify-center gap-1.5">
                                  <Calendar size={13} />
                                  {'Schedule Races'}
                                </button>
                                ...
                                return (
                                  <div className="space-y-2 w-full">
                                    <div className="text-red-400 font-bold text-[11px] flex items-center gap-1 bg-red-500/10 border border-red-500/20 rounded-lg px-2.5 py-1.5 animate-pulse">
                                      ...
                                      <span>
                                        {'Not enough qualified horses'} ({tour.qualifiedRegistration ?? 0}/12)
                                      </span>
                                    </div>
                                    <div className="flex gap-2 w-full">
                                      <button
                                        ...
                                        title={!canExtend ? (cancelCount >= 1 ? 'Already extended once, cannot extend again' : 'Tournament starts in less than 2 days, cannot extend') : ''}
                                      >
                                        <Clock size={13} />
                                        {'Extend'}
                                      </button>
                                      <button onClick={() => setCancelWarningTournament(tour)} className="flex-1 px-3 py-2 rounded-lg text-xs font-bold text-red-500 border border-red-500/50 bg-red-500/10 hover:bg-red-500/20 transition-colors flex items-center justify-center gap-1.5">
                                        {'Cancel Tournament'}
                                      </button>
                                    </div>
                                  </div>
                            ...
                            <div className="flex gap-2 w-full">
                              {show24hWarning ? (
                                ...
                                <button onClick={() => handleGenerateFinal(tour.tournamentId)} disabled={isGenerating} className="flex-1 px-3 py-2 rounded-lg text-xs font-bold text-gold border border-gold/30 bg-gold/10 hover:bg-gold/20 disabled:opacity-60 transition-colors flex items-center justify-center gap-1.5">
                                  ...
                                  {isGenerating ? 'Assigning...' : 'Auto Assign Final'}
                                </button>
                              ) : (
                                <button disabled className="flex-1 px-3 py-2 rounded-lg text-xs font-bold text-muted border border-glass-border bg-white/[0.04] cursor-not-allowed text-center">
                                  {'Scheduled & Ready'}
                                </button>
                              )}
                            </div>
                        ...
                        {customStatus === 'Pending Admin Attention' && (
                          <div className="flex flex-col gap-2.5 w-full">
                            <div className="text-red-400 font-bold text-[11px] flex items-start gap-1.5 bg-red-500/10 border border-red-500/20 rounded-lg px-2.5 py-1.5 animate-pulse">
                              ...
                              <span>{'Some races have no referee assigned. Please assign referees so the tournament can start!'}</span>
                            </div>
                            <div className="flex gap-2 w-full">
                              <button onClick={() => navigate('/admin/races', { state: { openTournamentId: tour.tournamentId } })} className="flex-1 px-3 py-2 rounded-lg text-xs font-bold text-red-400 border border-red-500/50 bg-red-500/10 hover:bg-red-500/20 animate-pulse transition-colors flex items-center justify-center gap-1.5">
                                <Calendar size={13} />
                                {'Assign referees'}
                              </button>
                            </div>
                        ...
                        {customStatus === 'Upcoming Registration' && (
                          <div className="flex gap-2 w-full">
                            <button onClick={() => setCancelWarningTournament(tour)} disabled={isGenerating} className="flex-1 px-3 py-2 rounded-lg text-xs font-bold text-red-500 border border-red-500/50 bg-red-500/10 hover:bg-red-500/20 disabled:opacity-60 transition-colors flex items-center justify-center gap-1.5">
                              {'Cancel'}
                            </button>
                          </div>
                        )}
                        ...
                        {customStatus !== 'Registration Open' && customStatus !== 'Registration Closed' && customStatus !== 'Scheduled' && customStatus !== 'Upcoming Registration' && (
                          <button disabled className="w-full px-3 py-2 rounded-lg text-xs font-bold text-muted border border-glass-border bg-white/[0.04] cursor-not-allowed text-center">
                            {customStatus}
                          </button>
                        )}

                        {customStatus !== 'Completed' && customStatus !== 'Cancelled' && (
                          <button onClick={() => handleEditClick(tour)} className="w-full mt-2 px-3 py-2 rounded-lg text-xs font-bold text-champagne border border-gold/30 bg-gold/5 hover:bg-gold/15 transition-colors flex items-center justify-center gap-1.5">
                            {'Edit Tournament'}
                          </button>
                        )}
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-gold/20 relative overflow-hidden">
            ...
            <div className="relative flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-serif text-white">{'Create new tournament'}</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.name} onChange={(e) => set('name', e.target.value)} placeholder={'E.g.: Autumn Race 2026'} className={INPUT} />
              </div>

              <div>
                <label className={LABEL}>{'Tournament Description'}</label>
                <textarea value={form.description} onChange={(e) => set('description', e.target.value)} placeholder={'Enter detailed tournament description...'} className={`${INPUT} h-20 resize-none`} />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={LABEL}>{'Open Registration *'}</label>
                  <div className="relative">
                    <input type="datetime-local" value={form.registrationStartDate} onChange={(e) => set('registrationStartDate', e.target.value)} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
                ...
                <div>
                  <label className={LABEL}>{'Close Registration *'}</label>
                  <div className="relative">
                    <input type="datetime-local" value={form.registrationEndDate} onChange={(e) => set('registrationEndDate', e.target.value)} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <input type="datetime-local" value={form.startDate} onChange={(e) => set('startDate', e.target.value)} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type="datetime-local" value={form.endDate} onChange={(e) => set('endDate', e.target.value)} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-gold/20 relative overflow-hidden">
                ...
                <div className="relative flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-serif text-white">{'Extend Registration'}</h2>
                  <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-xs px-4 py-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-300">Registration can be extended once. The new deadline is fixed at 48 hours before the tournament starts.</div>
                  ...
                  {!isValid && <div className="text-xs px-4 py-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 font-bold">The final 48-hour preparation window has started. The tournament can no longer be extended and must be cancelled.</div>}
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button onClick={handleExtendRegistration} disabled={!isValid || extendLoading} className="flex-1 btn-gold py-2.5 rounded-lg text-sm font-bold disabled:opacity-60 disabled:cursor-not-allowed">
                    {extendLoading ? 'Extending...' : 'Confirm'}
                  </button>
                </div>
              </motion.div>
            </div>
      ...
      {cancelWarningTournament &&
        (() => {
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-red-500/20 relative overflow-hidden text-left">
                ...
                <div className="relative flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-serif text-white">{'Cancel Tournament'}</h2>
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-xs px-4 py-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 font-bold">{(cancelWarningTournament.cancelCount ?? cancelWarningTournament.CancelCount ?? 0) >= 1 ? <span>The tournament registration has closed for the second time, but it failed to reach the minimum number of registered horses. Please click Cancel Tournament to complete the cancellation process.</span> : <span>Registration cannot be extended because the tournament starts in less than 48 hours (2 days). The tournament must be cancelled.</span>}</div>
                </div>

                <div className="flex gap-3 mt-6">
                  <button onClick={() => setCancelWarningTournament(null)} className="flex-1 py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                    {'Cancel'}
                  </button>
                  <button onClick={handleCancelTournament} disabled={cancelLoading} className="flex-1 py-2.5 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-bold disabled:opacity-60 disabled:cursor-not-allowed transition-colors">
                    {cancelLoading ? 'Cancelling...' : 'Cancel Tournament'}
                  </button>
                </div>
              </motion.div>
            </div>
      ...
      {editingTournament && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-gold/20 relative overflow-hidden text-left">
            <div className="absolute top-0 left-8 right-8 h-px bg-gradient-to-r from-transparent via-gold/40 to-transparent pointer-events-none" />
            <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-gradient-to-br from-gold/10 to-transparent blur-[40px] pointer-events-none" />
            <div className="relative flex items-center gap-3 mb-6">
              <div className="w-8 h-8 rounded-lg bg-gold/10 border border-gold/20 flex items-center justify-center shrink-0">
              ...
              <h2 className="text-xl font-serif text-white">{'Edit Tournament'}</h2>
              <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
            </div>
            ...
            <div className="space-y-4">
              <div>
                <label className={LABEL}>{'Tournament Name *'}</label>
                <input value={editForm.name} onChange={(e) => setEditForm((prev) => ({ ...prev, name: e.target.value }))} className={INPUT} />
              </div>

              <div>
                <label className={LABEL}>{'Tournament Description'}</label>
                <textarea value={editForm.description} onChange={(e) => setEditForm((prev) => ({ ...prev, description: e.target.value }))} className={`${INPUT} h-20 resize-none`} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={LABEL}>{'Open Registration *'}</label>
                  <div className="relative">
                    <input type="datetime-local" value={editForm.registrationStartDate} onChange={(e) => setEditForm((prev) => ({ ...prev, registrationStartDate: e.target.value }))} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
                ...
                <div>
                  <label className={LABEL}>{'Close Registration *'}</label>
                  <div className="relative">
                    <input type="datetime-local" value={editForm.registrationEndDate} onChange={(e) => setEditForm((prev) => ({ ...prev, registrationEndDate: e.target.value }))} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={LABEL}>{'Tournament Start *'}</label>
                  <div className="relative">
                    <input type="datetime-local" value={editForm.startDate} onChange={(e) => setEditForm((prev) => ({ ...prev, startDate: e.target.value }))} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
                ...
                <div>
                  <label className={LABEL}>{'Tournament End *'}</label>
                  <div className="relative">
                    <input type="datetime-local" value={editForm.endDate} onChange={(e) => setEditForm((prev) => ({ ...prev, endDate: e.target.value }))} className={`${INPUT} pr-10`} style={{ colorScheme: 'dark' }} />
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setEditingTournament(null)} className="flex-1 py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                {'Cancel'}
              </button>
