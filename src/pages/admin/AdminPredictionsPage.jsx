...
export function AdminPredictionsPage() {
  ...
  const getFilteredBets = () => {
    return bets.filter((b) => {
      ...
      const searchMatch = !search || b.spectatorName?.toLowerCase().includes(query) || b.horseName?.toLowerCase().includes(query) || b.raceName?.toLowerCase().includes(query);
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="max-w-[1600px] mx-auto px-8 py-6 space-y-6 relative z-10">
          <PageHero title="Bet Management" subtitle="Track and review spectator betting tickets, odds, and payouts" imageUrl="/images/hero-admin.jpg" imagePosition="center center" />

          {/* Stats */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {statsDisplay.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="glass-panel rounded-xl p-5 relative overflow-hidden">
                    ...
                    {pagedBets.map((b) => {
                      ...
                      return (
                        <tr key={b.betId} className="hover:bg-white/[0.01] transition-colors">
                          ...
                          <td className="px-6 py-4 text-xs text-muted">{b.createdAt ? new Date(b.createdAt).toLocaleString('vi-VN') : ''}</td>
                        </tr>
                      );
                    })}
                  </tbody>
