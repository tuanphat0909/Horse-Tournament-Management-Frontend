...
import { getAdminWalletBalance, getAdminWalletHistory, getDashboardStats, depositAdminWallet, withdrawAdminWallet } from '../../api/adminService';
...
export function AdminWalletPage() {
  ...
  async function loadAll() {
    ...
    setPageError('');
    try {
      const [balData, histData, statsData] = await Promise.all([getAdminWalletBalance(), getAdminWalletHistory(), getDashboardStats()]);
  ...
  async function handleActionSubmit(e) {
    ...
    try {
      ...
      setActionMessage({ type: 'error', text: parseApiError(err) });
    } finally {
    ...
    <div className="flex min-h-screen bg-[#070b13] text-white">
      ...
      <div className="flex-grow flex flex-col min-w-0">
        ...
        <main className="flex-grow p-8 overflow-y-auto">
          <div className="max-w-6xl mx-auto space-y-8">
            <PageHero title="Equestria Treasury & Wallet" subtitle="Monitor system revenues, house profits, betting payouts, and administrative wallet balances." imageUrl="/images/hero-admin.jpg" />

            {pageError && <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">{pageError}</div>}
            ...
            {pageLoading ? (
              ...
              <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <motion.div variants={child} className="glass-panel-elevated rounded-2xl p-6 border border-gold-border/30 relative overflow-hidden group">
                  ...
                  <div className="text-3xl font-extrabold text-white tracking-tight tabular-nums mb-3 relative z-10">${balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                ...
                <motion.div variants={child} className="glass-panel-elevated rounded-2xl p-6 border border-emerald-500/20 relative overflow-hidden group">
                  ...
                  <div className="text-3xl font-extrabold text-white tracking-tight tabular-nums mb-1">${(stats?.profit || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                ...
                <motion.div variants={child} className="glass-panel-elevated rounded-2xl p-6 border border-purple-500/20 relative overflow-hidden group">
                  ...
                  <div className="text-3xl font-extrabold text-white tracking-tight tabular-nums mb-1">${(stats?.totalRevenue || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-1.5 p-1 bg-white/[0.02] border border-glass-border rounded-xl">
                  {['all', 'deposit', 'withdraw', 'reward', 'other'].map((filter) => (
                    <button
                      key={filter}
                      onClick={() => {
                        setTxFilter(filter);
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all cursor-pointer ${txFilter === filter ? 'bg-gold text-rich-black' : 'text-muted hover:text-white hover:bg-white/[0.03]'}`}
                    >
                      {filter}
                    </button>
                  ))}
      ...
      {modalType && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-lg p-6 rounded-2xl glass-panel-elevated border border-gold-border/40 space-y-5 shadow-2xl relative my-8">
            <div className="flex items-center justify-between border-b border-glass-border/60 pb-4">
              <div className="flex items-center gap-3">
                <div className={`p-2.5 rounded-xl border ${modalType === 'deposit' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-blue-500/10 border-blue-500/20 text-blue-400'}`}>{modalType === 'deposit' ? <ArrowDownLeft size={22} /> : <ArrowUpRight size={22} />}</div>
                <div>
                  ...
                  <p className="text-[11px] text-muted">{modalType === 'deposit' ? 'Scan VietQR code or transfer funds directly to Admin Treasury bank account' : 'Withdraw liquidity from administrative treasury wallet to bank account'}</p>
                </div>
              </div>
              <button onClick={() => setModalType(null)} className="text-muted hover:text-white transition-colors cursor-pointer text-sm font-bold p-1 rounded-lg bg-white/5 hover:bg-white/10">
                ✕
              </button>
            </div>

            {actionMessage && <div className={`p-3.5 rounded-xl text-xs font-semibold ${actionMessage.type === 'success' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border border-red-500/20 text-red-400'}`}>{actionMessage.text}</div>}
            ...
            <form onSubmit={handleActionSubmit} className="space-y-5">
              ...
              <div>
                <label className="block text-xs font-semibold text-champagne mb-1.5 uppercase tracking-wider flex justify-between">
                  ...
                  <span className="text-gold font-normal font-mono text-[11px]">≈ {((parseFloat(amountInput) || 0) * 25000).toLocaleString('vi-VN')} VND</span>
                </label>
                <div className="relative">
                  <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gold font-bold text-sm">$</span>
                  <input type="number" step="0.01" min="0.01" placeholder="0.00" value={amountInput} onChange={(e) => setAmountInput(e.target.value)} required className="w-full pl-8 pr-4 py-2.5 rounded-xl bg-rich-black/60 border border-glass-border focus:border-gold/60 text-white font-mono text-sm outline-none transition-all shadow-inner" />
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                {[100, 500, 1000, 5000].map((preset) => (
                  <button key={preset} type="button" onClick={() => setAmountInput(preset.toString())} className="flex-1 py-1.5 rounded-lg bg-white/[0.03] hover:bg-gold/15 border border-glass-border/60 hover:border-gold/40 text-xs text-champagne font-mono font-semibold transition-all cursor-pointer">
                    +${preset}
                  </button>
                ))}
                {modalType === 'withdraw' && balance > 0 && (
                  <button type="button" onClick={() => setAmountInput(balance.toString())} className="px-3 py-1.5 rounded-lg bg-gold/20 hover:bg-gold/30 border border-gold/40 text-gold text-xs font-mono font-bold transition-all cursor-pointer">
                    Max (${balance})
                  </button>
                )}
              </div>
                  ...
                  <div className="bg-white/5 rounded-2xl p-4 border border-gold-border/30 flex flex-col md:flex-row items-center gap-4">
                    ...
                    <div className="flex flex-col items-center justify-center shrink-0">
                      <div className="p-2.5 bg-white rounded-xl border border-glass-border shadow-lg">
                        <img src={`https://img.vietqr.io/image/vietinbank-888899996666-print.png?amount=${Math.round((parseFloat(amountInput) || 100) * 25000)}&addInfo=DEPOSIT_TREASURY_ADMIN&accountName=EQUESTRIA%20TREASURY%20ADMIN`} alt="VietQR Admin Bank Deposit" className="w-36 h-36 object-contain" />
                      </div>
              ...
              {modalType === 'withdraw' && (
                <div className="bg-white/5 rounded-2xl p-4 border border-blue-500/30 text-xs space-y-3">
                  <div className="flex items-center justify-between py-1 border-b border-glass-border/30">
                    ...
                    <span className="text-gold font-mono font-bold text-sm">${balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-glass-border/40">
                <button type="button" onClick={() => setModalType(null)} className="px-4 py-2 rounded-xl text-xs font-semibold text-muted hover:text-white transition-colors cursor-pointer">
                  Cancel
                </button>
