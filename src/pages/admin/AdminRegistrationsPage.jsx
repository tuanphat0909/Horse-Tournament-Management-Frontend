...
export function AdminRegistrationsPage() {
  ...
  async function loadRegistrations() {
    ...
    try {
      ...
      const raw = Array.isArray(res) ? res : Array.isArray(res?.result) ? res.result : Array.isArray(res?.data?.result) ? res.data.result : Array.isArray(res?.data) ? res.data : [];
  ...
  async function handleReview(registrationId, status) {
    try {
      ...
      await (status === 'Approved' ? approveRegistration(registrationId) : rejectRegistration(registrationId));
      showToast(status === 'Approved' ? 'Approved successfully' : 'Rejected successfully', `Registration request #${registrationId} has been processed.`, 'success');
  ...
  const filteredRegistrations = registrations.filter((r) => {
    ...
    const searchMatch = !search || r.horseName?.toLowerCase().includes(query) || r.ownerName?.toLowerCase().includes(query) || r.tournamentName?.toLowerCase().includes(query);
  ...
  const sortedRegistrations = [...filteredRegistrations].sort((a, b) => {
    switch (sortBy) {
      case 'horse':
        return String(a.horseName ?? '').localeCompare(String(b.horseName ?? ''), 'vi');
      case 'oldest':
  ...
  const getCount = (t) => {
    ...
    return registrations.filter((r) => (r.status ?? '').toLowerCase() === statusVal && (tournamentFilter === 'all' || String(r.tournamentId) === tournamentFilter)).length;
  };

  const tournamentOptions = Array.from(new Map(registrations.map((registration) => [registration.tournamentId, registration.tournamentName])).entries()).sort((a, b) => String(a[1]).localeCompare(String(b[1])));
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 relative min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="Approve Registrations" subtitle="Review tournament registration requests from horse owners" imageUrl="/images/hero-admin.jpg" imagePosition="center center" />
          ...
          <div className="flex items-center gap-2 border-b border-glass-border pb-0">
            {['pending', 'approved', 'rejected', 'cancelled'].map((t) => {
              ...
              return (
                <button
                  ...
                  className={`px-5 py-3 text-sm font-medium border-b-2 -mb-px transition-all ${isActive ? `${cfg.color} border-current` : 'text-muted border-transparent hover:text-white'}`}
                >
                  {cfg.label}
                  <span className={`ml-2 px-2 py-0.5 rounded-full text-[11px] font-bold ${isActive ? cfg.bg + ' ' + cfg.color : 'bg-white/5 text-muted'}`}>{getCount(t)}</span>
                </button>
              );
            })}
            <div className="ml-auto mb-1 flex items-center gap-3">
              ...
              <button onClick={loadRegistrations} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-muted hover:text-white bg-white/[0.04] border border-glass-border hover:border-gold/30 transition-colors">
                <RefreshCw size={12} />
                Refresh
              </button>
          ...
          {error && (
            <div className="p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-lg text-sm">
              {error}
              <button onClick={loadRegistrations} className="ml-3 underline hover:no-underline">
                Retry
              </button>
            </div>
          )}
          ...
          {loading ? (
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-xl p-12 text-center relative overflow-hidden">
              ...
              <div className="text-muted text-sm">{registrations.length === 0 ? 'No registrations in the system yet' : `No registrations with status "${TAB_CONFIG[tab].label}"`}</div>
            </motion.div>
          ) : (
                  ...
                  <tbody className="divide-y divide-glass-border/40 text-sm text-white">
                    {pagedRegistrations.map((reg) => (
                      <tr key={reg.registrationId} className="hover:bg-white/[0.01] transition-colors">
                        ...
                        <td className="px-6 py-4">{reg.jockeyName ? <span className="text-white font-medium flex items-center gap-1.5">🏇 {reg.jockeyName}</span> : <span className="text-amber-400/90 text-xs font-semibold px-2 py-0.5 rounded bg-amber-500/10 border border-amber-500/20">Missing Jockey (Not Assigned)</span>}</td>
                        <td className="px-6 py-4 text-muted">{reg.ownerName}</td>
                        <td className="px-6 py-4 text-muted">{reg.tournamentName}</td>
                        <td className="px-6 py-4 text-muted">{reg.registeredAt ? new Date(reg.registeredAt).toLocaleDateString('vi-VN') : '—'}</td>
                        <td className="px-6 py-4">{reg.status === 'Pending' ? reg.jockeyContractStatus === 'Accepted' ? <span className="px-2 py-1 rounded text-xs font-semibold bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">Awaiting Approval</span> : <span className="px-2 py-1 rounded text-xs font-semibold bg-slate-500/10 text-slate-400 border border-slate-500/20">Awaiting Jockey acceptance</span> : <span className={`px-2 py-1 rounded text-xs font-semibold ${reg.status === 'Approved' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : reg.status === 'Cancelled' ? 'bg-slate-500/10 text-slate-400 border border-slate-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>{reg.status === 'Approved' ? 'Approved' : reg.status === 'Cancelled' ? 'Cancelled — participant list finalized' : 'Declined'}</span>}</td>
                        {tab === 'pending' && (
                          <td className="px-6 py-4">
                            <div className="flex items-center justify-center gap-2">
                              <button onClick={() => handleReview(reg.registrationId, 'Approved')} disabled={processingId === reg.registrationId || reg.jockeyContractStatus !== 'Accepted'} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                                <Check size={12} />
                                Approve
                              </button>
                              <button onClick={() => handleReview(reg.registrationId, 'Rejected')} disabled={processingId === reg.registrationId} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                                <X size={12} />
                                Reject
                              </button>
                            </div>
