...
export function AdminRefereesPage() {
  ...
  const fetchData = async () => {
    try {
      const [refereesRes, racesRes] = await Promise.all([getReferees(), getRacesRefereeAssignments()]);

      const refereesData = refereesRes?.result ?? (Array.isArray(refereesRes) ? refereesRes : []);
      setReferees(refereesData.map((r) => ({ ...r, fullName: fixMojibake(r.fullName) })));
      ...
      setRaces(
        racesData.map((race) => ({
          ...
          referees: (race.referees ?? []).map((ref) => ({ ...ref, fullName: fixMojibake(ref.fullName) })),
        }))
      );
    } catch (err) {
  ...
  const handleRemove = async (raceId, refereeId) => {
  ...
  const filteredReferees = referees.filter((r) => {
    ...
    return (r.fullName ?? '').toLowerCase().includes(q) || (r.email ?? '').toLowerCase().includes(q) || (r.licenseNumber ?? '').toLowerCase().includes(q);
  });
        ...
        <main className="max-w-[1600px] mx-auto px-8 py-6 space-y-6 relative z-10">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-6">
              ...
              <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} className="glass-panel rounded-xl overflow-hidden h-fit">
                <div className="p-5 border-b border-glass-border flex items-center gap-2" style={{ backgroundColor: 'rgba(255, 255, 255, 0.02)' }}>
                  ...
                  <span className="ml-auto px-2.5 py-0.5 rounded-full bg-gold/10 text-gold text-xs font-bold border border-gold/20">{filteredReferees.length}</span>
                </div>
                <div className="px-5 pt-4">
                  <div className="flex items-center gap-2 bg-white/[0.04] border border-glass-border rounded-lg px-3 py-2">
                    <Search size={13} className="text-muted shrink-0" />
                    <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search referees by name, email, license..." className="bg-transparent text-sm text-white placeholder:text-muted/60 outline-none w-full" />
                  </div>
                </div>
                <div className="p-5">
                  {filteredReferees.length === 0 ? (
                    ...
                    <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
                      {filteredReferees.map((r) => {
                        ...
                        return (
                          <div key={r.refereeId} className="p-3 rounded-lg bg-white/[0.02] border border-glass-border hover:border-gold/20 transition-all">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-full bg-gold/10 border border-gold/20 flex items-center justify-center shrink-0 text-gold text-sm font-bold font-serif">{r.fullName ? r.fullName.charAt(0).toUpperCase() : 'R'}</div>
                              ...
                              {assigns.length > 0 ? <span className="shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{assigns.length} races</span> : <span className="shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/5 text-muted border border-glass-border">Unassigned</span>}
                            </div>
                                ...
                                {assigns.map((a, i) => (
                                  <div key={i} className="flex items-center gap-2 text-[11px]">
                                    ...
                                    <span className="text-muted truncate">
                                      • {a.roundName || 'Round ?'} • {a.tournamentName || 'Tournament ?'}
                                    </span>
              ...
              <div className="space-y-6">
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-xl overflow-hidden">
                  <div className="p-5 border-b border-glass-border flex items-center gap-2" style={{ backgroundColor: 'rgba(255, 255, 255, 0.02)' }}>
                    ...
                    <span className="ml-auto px-2.5 py-0.5 rounded-full bg-yellow-500/10 text-yellow-400 text-xs font-bold border border-yellow-500/20">{unassignedRaces.length}</span>
                  </div>
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {unassignedRaces.map((race) => (
                            <div key={race.raceId} className="p-4 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/10 transition-all flex flex-col justify-between">
                              <div>
                                <div className="flex items-start justify-between gap-2 mb-2">
                                  <span className="text-[10px] font-bold uppercase tracking-wider text-gold/80 bg-gold/10 px-2 py-0.5 rounded border border-gold/20">{race.roundName || 'Prefinal'}</span>
                              ...
                              <div className="pt-3 mt-3 border-t border-glass-border">
                                <button onClick={() => navigate('/admin/races')} className="w-full py-1.5 rounded bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 hover:bg-cyan-500/20 transition-all text-xs font-semibold flex items-center justify-center gap-1.5">
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-panel rounded-xl overflow-hidden">
                  <div className="p-5 border-b border-glass-border flex items-center gap-2" style={{ backgroundColor: 'rgba(255, 255, 255, 0.02)' }}>
                    ...
                    <span className="ml-auto px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-bold border border-emerald-500/20">{assignedRaces.length}</span>
                  </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {assignedRaces.map((race) => (
                          <div key={race.raceId} className="p-4 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/10 transition-all flex flex-col justify-between">
                            <div>
                              <div className="flex items-start justify-between gap-2 mb-2">
                                <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">{race.roundName || 'Prefinal'}</span>
                                <span className="text-xs text-muted flex items-center gap-1">
                                  <Calendar size={12} />
                                  {new Date(race.raceDate).toLocaleDateString('vi-VN')}
                                </span>
                            ...
                            <div className="pt-3 border-t border-glass-border space-y-2">
                              ...
                              <div className="space-y-1.5">
                                {race.referees.map((ref) => (
                                  <div key={ref.refereeId} className="flex items-center justify-between p-2 rounded bg-white/[0.01] border border-white/5 text-xs">
                                    <div className="min-w-0 flex-1">
                                    ...
                                    <button className="p-1 rounded text-muted hover:text-red-400 hover:bg-red-500/10 transition-all" onClick={() => handleRemove(race.raceId, ref.refereeId)} title="Cancel assignment">
                                      <Trash2 size={13} />
                                    </button>
                                  </div>
                                ))}
