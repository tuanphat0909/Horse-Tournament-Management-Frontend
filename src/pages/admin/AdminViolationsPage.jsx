...
function ViolationRow({ v, processingId, handleReview }) {
  ...
  return (
    <tr className="hover:bg-white/[0.01] transition-colors">
      ...
      <td className="px-6 py-4">
        <span className="px-2 py-1 bg-red-500/10 text-red-400 border border-red-500/20 rounded text-xs font-semibold">{v.penalty}</span>
      </td>
        ...
        {isCountdownActive ? (
          ...
          <div className="flex flex-col items-end gap-1.5">
            <div className="flex items-center justify-end gap-2">
              <button onClick={() => handleReview(v.violationId, 'Confirmed')} disabled={processingId === v.violationId} className="px-3 py-1.5 rounded bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 transition-all flex items-center gap-1 text-xs font-semibold disabled:opacity-40" title="Confirm Violation">
                <Check size={12} />
                <span>Confirm</span>
              </button>
              <button onClick={() => handleReview(v.violationId, 'Rejected')} disabled={processingId === v.violationId} className="px-3 py-1.5 rounded bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 transition-all flex items-center gap-1 text-xs font-semibold disabled:opacity-40" title="Dismiss violation">
                <X size={12} />
                <span>Dismiss</span>
              </button>
            </div>
            {createdAtMs && <div className="text-[10px] text-emerald-400/90 font-medium">Ready: You can now confirm or dismiss.</div>}
          </div>
        )}
      </td>
    </tr>
...
export function AdminViolationsPage() {
  ...
  const fetchViolations = async () => {
    try {
      ...
      const raw = Array.isArray(res) ? res : Array.isArray(res?.result) ? res.result : [];
      setViolations(raw);
      setError('');
    } catch (err) {
      console.error(err);
  ...
  const activeList = (tab === 'pending' ? pendingViolations : tab === 'confirmed' ? confirmedViolations : rejectedViolations).filter((v) => {
    if (!search.trim()) return true;
    const q = search.toLowerCase();
    return (v.raceName ?? '').toLowerCase().includes(q) || (v.type ?? '').toLowerCase().includes(q) || (v.note ?? '').toLowerCase().includes(q);
  });
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="max-w-[1600px] mx-auto px-8 py-6 space-y-6 relative z-10">
          <PageHero title="Handle Violations & Appeals" subtitle="Review violations recorded by referees and decide to confirm or dismiss them" imageUrl="/images/hero-admin.jpg" imagePosition="center center" />
            ...
            <div className="relative z-10 flex items-center gap-2 text-xs flex-wrap">
              ...
              {[
                ...
                s.sep ? (
                  ...
                  <span key={i} className={`px-2.5 py-1 rounded-lg border text-white/80 ${s.active ? 'bg-gold/10 border-gold/20 text-gold font-bold' : s.note ? 'bg-blue-500/10 border-blue-500/20 text-blue-400' : 'bg-white/[0.03] border-glass-border'}`}>
                    {s.label}
                  </span>
                )
              )}
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} className="glass-panel rounded-xl p-5 relative overflow-hidden border border-glass-border hover:border-gold/30 transition-all">
                ...
                <div className="relative z-10 text-2xl text-white font-extrabold tracking-tight tabular-nums">{s.value}</div>
                      ...
                      <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-glass-border/40 text-sm text-white">
                          {pagedViolations.map((v) => (
                            <ViolationRow key={v.violationId} v={v} processingId={processingId} handleReview={handleReview} />
                          ))}
                        </tbody>
                      </table>
                    </div>
