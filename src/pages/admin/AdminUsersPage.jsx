...
const accountSchema = Yup.object({
  ...
  experienceYears: Yup.string()
    .test('valid-years', 'Years of experience must be a whole number of 0 or more.', (value) => {
      if (!value) return true;
      const years = Number(value);
      return Number.isInteger(years) && years >= 0;
    })
    .test('not-too-large', 'Years of experience looks too large.', (value) => !value || Number(value) <= 80),
});
...
export function AdminUsersPage() {
  ...
  useEffect(() => {
    ...
    getRoles()
      .then((data) => {
        const raw = Array.isArray(data) ? data : Array.isArray(data.result) ? data.result : [];
        setRoles(raw.map((r) => (typeof r === 'string' ? r : (r.name ?? ''))).filter(Boolean));
      })
      .catch(() => setRoles([]));
  }, [showModal]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (validationError instanceof Yup.ValidationError) {
        validationError.inner.forEach((e) => {
          if (e.path && !invalid[e.path]) {
            invalid[e.path] = e.message;
          }
        });
      }
    ...
    try {
      ...
      const fromBody = parseFieldErrors(err);
      const matched = mapMessageToField(message);
      setFieldErrors(Object.keys(fromBody).length > 0 ? fromBody : matched ? { [matched]: message } : {});
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const filteredAccounts = accounts.filter((acc) => {
    ...
    if (search.trim()) {
      ...
      return acc.fullName?.toLowerCase().includes(q) || acc.email?.toLowerCase().includes(q);
    }
    return true;
  });
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
            ...
            {['all', 'admin', 'owner', 'jockey', 'referee', 'spectator', 'vet'].map((r) => (
              ...
              >
                ...
                <div className="relative z-10 flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] uppercase tracking-wider text-muted font-bold">{r === 'all' ? 'All' : ROLE_LABELS[r]}</span>
                </div>
                <div className="relative z-10 text-xl font-serif font-bold text-white">{counts[r]}</div>
              </button>
            ))}
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-panel rounded-xl overflow-hidden relative">
            ...
            <div className="relative p-5 border-b border-glass-border flex items-center gap-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-white/[0.04] border border-glass-border text-muted">
                <span className="text-champagne font-semibold">{filteredAccounts.length}</span> results
              </span>
                    ...
                    {pagedAccounts.map((user) => (
                      <tr key={user.userId} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="py-4 px-6">
                          <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold border ${user.roleName === 'Admin' ? 'text-gold bg-gold/10 border-gold/20' : user.roleName === 'HorseOwner' ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' : user.roleName === 'Jockey' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : user.roleName === 'Referee' ? 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20' : 'text-gray-400 bg-gray-500/10 border-gray-500/20'}`}>{ROLE_LABELS[user.roleName?.toLowerCase()] ?? user.roleName}</span>
                        </td>
                        <td className="py-4 px-6">
                          <span className={`inline-flex items-center gap-1.5 text-xs font-semibold ${user.status === 'Active' ? 'text-emerald-400' : 'text-red-400'}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${user.status === 'Active' ? 'bg-emerald-400' : 'bg-red-400'}`} />
                            {user.status}
                          </span>
                        </td>
                        <td className="py-4 px-6 text-muted">{formatUtcDateTime(user.createdAt)}</td>
                        <td className="py-4 px-6 text-right">
                          <button onClick={() => handleToggleStatus(user.userId, user.status)} disabled={user.roleName === 'Admin' || togglingId === user.userId} className={`px-3 py-1.5 rounded text-xs font-bold transition-colors ${user.roleName === 'Admin' ? 'opacity-30 cursor-not-allowed bg-glass-border/30 text-muted' : user.status === 'Active' ? 'bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20' : 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/20'}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-gold/20 max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input value={form.password} onChange={(e) => set('password', e.target.value)} type={showPwd ? 'text' : 'password'} placeholder="At least 6 characters" className={(fieldErrors.password ? INPUT_ERROR : INPUT) + ' pr-10'} />
              ...
              <div>
                ...
                {roles.length === 0 ? (
                  ...
                  <select value={form.role} onChange={(e) => set('role', e.target.value)} className={fieldErrors.role ? INPUT_ERROR : INPUT} style={{ colorScheme: 'dark' }}>
              ...
              {/* Error / Success */}
              {error && <div className="text-sm px-4 py-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 whitespace-pre-line">{error}</div>}
