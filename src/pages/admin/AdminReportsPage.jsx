...
export function AdminReportsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return reports.filter((report) => {
      ...
      return [report.raceName, report.tournamentName, report.refereeName, report.content, report.violationNote, report.reportedHorseName, report.reportedUserName].some((value) =>
        String(value ?? '')
          .toLowerCase()
          .includes(query)
      );
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="Referee Reports" subtitle="Review reports submitted by assigned referees" imageUrl="/images/hero-admin.jpg" imagePosition="center center" />

          <div className="glass-panel rounded-xl border border-glass-border p-4 flex flex-col md:flex-row gap-3">
            <div className="relative flex-1">
              <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted" />
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search referee, race, horse or report content..." className="w-full bg-navy/50 border border-glass-border rounded-lg pl-10 pr-4 py-2.5 text-sm text-white outline-none focus:border-gold/40" />
            </div>
            <select value={tournamentId} onChange={(event) => setTournamentId(event.target.value)} className="bg-navy/50 border border-glass-border rounded-lg px-4 py-2.5 text-sm text-white outline-none focus:border-gold/40" style={{ colorScheme: 'dark' }}>
              ...
              {paged.map((report) => (
                <article key={report.reportId} className="glass-panel rounded-xl border border-glass-border p-5 space-y-4 hover:border-gold/25 transition-colors">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="text-xs text-muted flex items-center gap-1.5 mt-1">
                        <Trophy size={12} />
                        {report.tournamentName || `Tournament #${report.tournamentId}`}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 text-sm text-champagne">
                    ...
                    <span>
                      Submitted by <b>{report.refereeName}</b>
                    </span>
                  </div>

                  <div className="rounded-lg bg-white/[0.025] border border-glass-border/70 p-4 text-sm text-body whitespace-pre-wrap leading-relaxed">{report.content}</div>
                  ...
                  {(report.violationNote || report.reportedHorseName || report.reportedUserName) && (
                    <div className="rounded-lg bg-red-500/[0.06] border border-red-500/20 p-3 text-xs space-y-1.5">
                      ...
                      {report.reportedHorseName && (
                        <div className="text-muted">
                          Horse: <span className="text-white">{report.reportedHorseName}</span>
                        </div>
                      )}
                      {report.reportedUserName && (
                        <div className="text-muted">
                          User: <span className="text-white">{report.reportedUserName}</span>
                        </div>
                      )}
                      {report.violationNote && (
                        <div className="text-muted whitespace-pre-wrap">
                          Note: <span className="text-white">{report.violationNote}</span>
                        </div>
                      )}
                    </div>
                  )}
          ...
          {!loading && !error && total > 0 && <Pager page={safePage} totalPages={totalPages} total={total} onChange={setPage} />}
        </main>
      </div>
    </div>
  );
