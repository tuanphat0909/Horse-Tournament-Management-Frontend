...
export function AdminResultsPage() {
  ...
  async function toggleExpandRace(raceId) {
  ...
  async function handlePublish(raceId) {
  ...
  function handleTournamentChange(tournamentIdStr) {
    ...
    if (selectedTour && selectedTour.prizes && selectedTour.prizes.length > 0) {
      const p1 = selectedTour.prizes.find((p) => p.rankPosition === 1)?.amount ?? '';
      const p2 = selectedTour.prizes.find((p) => p.rankPosition === 2)?.amount ?? '';
      const p3 = selectedTour.prizes.find((p) => p.rankPosition === 3)?.amount ?? '';
  ...
  async function handleCreatePrizes() {
    ...
    try {
      ...
      setPrizesError(parseApiError(err));
    } finally {
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="max-w-[1600px] mx-auto px-8 py-6 space-y-6 relative z-10">
          <PageHero title="Results & Publishing" subtitle="Confirm and publish official results" imageUrl="/images/hero-admin.jpg" imagePosition="center center" />
          ...
          <div className="flex items-center gap-2 bg-white/[0.04] border border-glass-border focus-within:border-gold/40 transition-colors rounded-lg px-3 py-2 w-64">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search races, tournaments..." className="bg-transparent text-sm text-white placeholder:text-muted/60 outline-none w-full" />
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="glass-panel rounded-xl p-6 border border-glass-border hover:border-gold/30 transition-all relative overflow-hidden">
              ...
              <button onClick={() => setShowPrizesModal(true)} className="btn-gold w-full py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 relative z-10">
          ...
          <div>
            ...
            {loadingRaces ? (
              ...
              (() => {
                const matchesSearch = (r) => !search.trim() || (r.name ?? r.raceName ?? '').toLowerCase().includes(search.toLowerCase()) || (r.tournamentName ?? '').toLowerCase().includes(search.toLowerCase());
                        ...
                        <div key={race.raceId} className="glass-panel rounded-xl border border-glass-border overflow-hidden flex flex-col justify-between">
                          <div className="p-5 flex items-center justify-between gap-4">
                            <div>
                              ...
                              <div className="text-xs text-muted">
                                {cleanDisplayString(race.name ?? race.raceName)} • {new Date(race.raceDate || race.startTime).toLocaleString('vi-VN')}
                              </div>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <button onClick={() => toggleExpandRace(race.raceId)} className="px-2.5 py-1.5 rounded-lg border border-glass-border text-xs text-muted hover:text-white hover:bg-white/5 transition-all">
                          ...
                          {isExpanded && (
                            <div className="border-t border-glass-border/30 bg-navy/60 p-4 text-xs space-y-3">
                              <div className="font-bold text-[10px] text-muted uppercase tracking-wider">Results recorded by the system (for verification):</div>
                              {loadingEntries ? (
                                <LoadingSkeleton />
                                    ...
                                    <tbody className="divide-y divide-glass-border/20 text-white/90">
                                      {raceEntries.map((entry) => (
                                        <tr key={entry.raceEntryId}>
                                          <td className="py-2 pr-3 font-bold text-gold">{entry.finishPosition ? `${entry.finishPosition}` : '-'}</td>
                                          ...
                                          <td className="py-2 text-right font-mono">{entry.finishTime ? `${entry.finishTime}s` : '-'}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
          ...
          <div>
            ...
            {loadingRaces ? (
              ...
              (() => {
                const publishedRaces = races.filter((r) => (r.status === 'Published' || r.status === 'Finished') && (!search.trim() || (r.name ?? r.raceName ?? '').toLowerCase().includes(search.toLowerCase()) || (r.tournamentName ?? '').toLowerCase().includes(search.toLowerCase())));
                    ...
                    {publishedRaces.map((race) => {
                      ...
                      return (
                        <div key={race.raceId} className="glass-panel rounded-xl border border-glass-border overflow-hidden flex flex-col justify-between">
                          ...
                          {isExpanded && (
                            <div className="border-t border-glass-border/30 bg-navy/60 p-4 text-xs space-y-3">
                              <div className="font-bold text-[10px] text-muted uppercase tracking-wider">Official results:</div>
                              {loadingEntries ? (
                                <LoadingSkeleton />
                              ) : raceEntries.length === 0 ? (
                                <div className="text-muted/60 italic py-2">No race entry data</div>
      ...
      {showPrizesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-8 w-full max-w-lg border border-gold/20 relative overflow-hidden">
            <button onClick={closePrizesModal} className="absolute top-4 right-4 p-1 rounded-lg text-muted hover:text-white hover:bg-white/5 transition-all z-20" title="Close">
              <X size={18} />
            </button>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={prizes.tournamentId} onChange={(e) => handleTournamentChange(e.target.value)} className={INPUT + ' cursor-pointer'} style={{ colorScheme: 'dark' }}>
                  <option value="" className="text-muted/60">
                    -- Select Tournament --
                  </option>
                  {tournaments.map((t) => (
              ...
              {prizes.tournamentId &&
                (() => {
                  ...
                  return hasPrizes ? <div className="text-xs px-3 py-2 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-medium leading-relaxed">✓ This tournament already has prizes configured. The previously configured amounts are shown below. You can modify them if needed.</div> : <div className="text-xs px-3 py-2 rounded bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 font-medium leading-relaxed">⚠ This tournament has no prizes configured yet. Please set them up below.</div>;
                })()}
              <div>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={closePrizesModal} className="flex-1 py-2.5 rounded-lg border border-glass-border text-muted hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                Cancel
              </button>
