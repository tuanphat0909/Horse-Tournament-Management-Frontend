...
const RANK_STYLE = ['bg-gradient-to-br from-[#FFE07A] to-[#F5A623] text-[#2A1D00] border-gold/40', 'bg-gradient-to-br from-slate-200 to-slate-400 text-slate-800 border-slate-300/40', 'bg-gradient-to-br from-amber-600 to-amber-800 text-amber-50 border-amber-600/40'];
...
export function LeaderboardPage() {
  ...
  useEffect(() => {
    async function load() {
      const [h, j, schedule] = await Promise.allSettled([getHorseRankings(), getJockeyRankings(), getRaceSchedule()]);
      const pick = (r) => (r.status === 'fulfilled' ? (r.value?.result ?? (Array.isArray(r.value) ? r.value : [])) : []);
      ...
      const finishedRaces = pick(schedule).filter((r) => ['finished', 'completed', 'published'].includes(String(r.status ?? '').toLowerCase()));
      const entryLists = await Promise.allSettled(finishedRaces.map((r) => getRaceEntries(r.raceId ?? r.id)));
      ...
      for (const res of entryLists) {
        ...
        for (const e of Array.isArray(entries) ? entries : []) {
          ...
          for (const [map, id] of [
            [horseStats, e.horseId],
            [jockeyStats, e.jockeyId],
          ]) {
            if (id == null) continue;
    ...
    <div className="min-h-screen bg-navy text-body font-sans selection:bg-gold/30">
      ...
      <main className="max-w-6xl mx-auto px-6 pt-32 pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="landing-section text-center mb-12">
          ...
          <p className="text-sm text-muted max-w-xl mx-auto">Live standings from every official race in the Equestria system — updated as soon as referees confirm the results.</p>
        </motion.div>
        ...
        <div className="landing-section flex justify-center gap-2 mb-8">
          {[
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all flex items-center gap-2 border ${tab === key ? 'bg-gold/15 border-gold/40 text-champagne' : 'border-glass-border text-muted hover:text-white hover:bg-white/[0.04]'}`}
            >
              <Icon size={15} /> {label}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-2xl overflow-hidden border border-glass-border">
                    ...
                    return (
                      <tr key={r.horseId ?? r.jockeyId ?? rank} className="hover:bg-white/[0.02] transition-colors">
                        <td className="px-6 py-4">
                          <span className={`w-8 h-8 rounded-full border flex items-center justify-center text-xs font-bold ${rank < 3 ? RANK_STYLE[rank] : 'bg-white/[0.03] border-glass-border text-muted'}`}>{rank < 3 ? <Medal size={14} /> : rank + 1}</span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="font-serif font-bold text-white">{name}</div>
                          {tab === 'horses' && r.breed && <div className="text-[11px] text-muted mt-0.5">{r.breed}</div>}
                        </td>
                        <td className="px-6 py-4 text-muted">{tab === 'horses' ? r.ownerName || '—' : r.experienceYears != null ? `${r.experienceYears} yrs` : '—'}</td>
                        ...
                        <td className="px-6 py-4 text-right">
                          ...
                          {r.winRate == null ? <span className="text-muted/50">—</span> : <span className={`font-mono font-bold ${r.winRate >= 50 ? 'text-emerald-400' : 'text-muted'}`}>{r.winRate}%</span>}
                        </td>
                      </tr>
                    );
                  })}
            ...
            {totalPages > 1 && <Pager page={safePage} totalPages={totalPages} total={total} onChange={setPage} />}
          </motion.div>
        )}
