...
export function RefereeConfirmResultsPage() {
  ...
  useEffect(() => {
    getRefereeDashboard()
      .then((data) => {
        ...
        const activeOrFinishedRaces = list.filter((r) => {
  ...
  async function handleRaceChange(raceId) {
    ...
    try {
      ...
      const hasTimes = sorted.some((e) => e.finishTime != null);
      const baseWinnerTime = Math.round(55 + Math.random() * 8);

      const origMap = {};

      sorted.forEach((e, index) => {
        const timeVal = hasTimes && e.finishTime != null ? Number(e.finishTime) : Number((baseWinnerTime + index * 1.4 + Math.random() * 1.5).toFixed(2));
  ...
  function handleTimeChange(raceEntryId, value) {
  ...
  async function executeSubmit() {
    ...
    try {
      ...
      const sorted = [...raceEntries].sort((a, b) => (a.finishTime ?? 999999) - (b.finishTime ?? 999999));
      sorted.forEach((e, idx) => {
        e.finishPosition = idx + 1;
      });
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 relative min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="Confirm Results" subtitle="Confirm official race times and publish leaderboard" imageUrl="/images/hero-referee.jpg" imagePosition="right 28%" />
            ...
            <div className={`${form.raceId ? 'lg:col-span-5' : 'w-full'} glass-panel rounded-xl p-8 border border-glass-border h-fit`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input readOnly value={form.winner ? `🏆 ${form.winner}` : '-- Select Race First --'} className={READONLY_INPUT} />
                ...
                <div>
                  ...
                  <input readOnly value={form.winningTime ? `${form.winningTime} seconds` : '-- Select Race First --'} className={READONLY_INPUT + ' text-emerald-400'} />
                </div>
                ...
                <div className="pt-2">
                  <button type="submit" disabled={loading || !form.raceId || !!isCompleted} className="btn-red w-full py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
            ...
            {form.raceId && (
              <div className="lg:col-span-7 glass-panel rounded-xl p-8 border border-glass-border flex flex-col h-fit">
                <div className="flex items-center justify-between mb-6 pb-3 border-b border-glass-border/40">
                  ...
                  <button type="button" onClick={handleSortAndSaveLeaderboard} disabled={!!isCompleted} className="btn-gold px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all shadow-md">
                          ...
                          return (
                            <tr key={entry.raceEntryId || index} className="hover:bg-white/[0.01] transition-colors">
                              ...
                              <td className="py-3.5 pr-3 text-center">
                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${index === 0 ? 'bg-gold/20 text-gold border border-gold/40' : index === 1 ? 'bg-slate-300/20 text-slate-200 border border-slate-300/30' : index === 2 ? 'bg-amber-700/20 text-amber-300 border border-amber-700/30' : 'bg-white/5 text-muted border border-glass-border'}`}>Rank #{index + 1}</span>
                              </td>
                              ...
                              <td className="py-3.5 text-right">
                                <div className="flex items-center justify-end gap-1.5">
                                  ...
                                  <input disabled={!!isCompleted} type="number" step="0.01" min="0" value={entry.finishTime ?? ''} onChange={(e) => handleTimeChange(entry.raceEntryId, e.target.value)} className="w-28 bg-navy/60 border border-glass-border rounded-lg px-3 py-1.5 text-sm text-white text-right font-mono focus:border-gold/50 outline-none" />
                                </div>
                              </td>
                            </tr>
                          );
          ...
          <AnimatePresence>
            {showReasonModal && (
              <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="glass-panel rounded-2xl p-7 w-full max-w-lg border border-gold/30 relative overflow-hidden">
                      ...
                      <div key={h.raceEntryId} className="pt-3 first:pt-0">
                        <div className="flex justify-between items-center text-xs mb-1.5">
                          <span className="font-bold text-white">
                            🐎 {h.horseName} (Lane {h.laneNo})
                          </span>
                          <span className="text-muted font-mono">
                            {originalTimes[h.raceEntryId]}s ➔ <span className="text-gold font-bold">{h.finishTime}s</span>
                          </span>
                        </div>
                        <input type="text" value={timeReasons[h.raceEntryId] || ''} onChange={(e) => setTimeReasons({ ...timeReasons, [h.raceEntryId]: e.target.value })} placeholder="Reason (e.g. Photo finish camera review, obstacle timing adjustment)..." className="w-full bg-navy/60 border border-glass-border rounded-lg px-3 py-2 text-xs text-white outline-none focus:border-gold/40" />
                      </div>
                    ))}
                  </div>

                  <div className="flex justify-end gap-3 pt-4 border-t border-glass-border/40">
                    <button onClick={() => setShowReasonModal(false)} className="px-5 py-2 rounded-xl text-xs text-muted border border-glass-border hover:text-white transition-colors">
                      Cancel
                    </button>
                    <button onClick={executeSubmit} className="btn-gold px-6 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5">
