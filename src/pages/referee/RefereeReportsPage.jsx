...
export function RefereeReportsPage() {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      if (res && res.result) {
        ...
        const now = new Date();
        const activeOrFinishedRaces = assigned.filter((r) => {
  ...
  async function loadRaceData() {
    ...
    try {
      const [repRes, horseRes] = await Promise.all([getRaceReports(selectedRaceId), getHorseChecks(selectedRaceId)]);
  ...
  // Form tạo báo cáo — Formik validate nội dung; race lấy từ selectedRaceId (dùng chung
  // với dropdown ngoài trang nên giữ ở state riêng), vẫn bắt buộc chọn race trước khi gửi.
  const reportFormik = useFormik({
    initialValues: { content: '', violationNote: '', reportedHorseId: '' },
    ...
    onSubmit: async (values) => {
      ...
      try {
        const payload = {
          ...
          violationNote: values.violationNote || null,
          reportedHorseId: values.reportedHorseId ? Number(values.reportedHorseId) : null,
        };
          ...
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              {loadingRaces ? (
                ...
                <select value={selectedRaceId} onChange={(e) => setSelectedRaceId(Number(e.target.value))} className="bg-white/[0.04] border border-glass-border rounded-lg px-3 py-2 text-sm text-white focus:border-gold outline-none min-w-[200px]">
                  {races.map((r) => (
                  ...
                  <div className="space-y-4">
                    {reports.map((rep) => (
                      <div key={rep.reportId} className="bg-white/[0.02] border border-glass-border/60 hover:border-gold/20 p-4 rounded-xl space-y-3 transition-all">
                        ...
                        <div className="text-sm text-white/90 whitespace-pre-wrap leading-relaxed">{rep.content}</div>
          ...
          {showAdd && (
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel rounded-2xl p-7 w-full max-w-lg border border-glass-border relative overflow-hidden">
                ...
                <form onSubmit={reportFormik.handleSubmit} noValidate className="relative z-10">
                  <div className="space-y-4">
                  <div>
                    <label className={LABEL}>Select Race *</label>
                    <select value={selectedRaceId} onChange={(e) => setSelectedRaceId(Number(e.target.value))} className={INPUT} style={{ colorScheme: 'dark' }}>
                  ...
                  <div>
                    ...
                    <select name="reportedHorseId" value={reportFormik.values.reportedHorseId} onChange={reportFormik.handleChange} disabled={!selectedRaceId} className={INPUT + ' disabled:opacity-50'} style={{ colorScheme: 'dark' }}>
                  ...
                  {reportFormik.values.reportedHorseId && (
                    <div>
                      <label className={LABEL}>Horse Violation Notes *</label>
                      <input name="violationNote" value={reportFormik.values.violationNote} onChange={reportFormik.handleChange} placeholder="Describe violating behavior..." className={INPUT} />
                    </div>
                  )}

                  <div>
                    <label className={LABEL}>Detailed Report Content *</label>
                    <textarea rows={5} name="content" value={reportFormik.values.content} onChange={reportFormik.handleChange} onBlur={reportFormik.handleBlur} placeholder="Describe details of the race progress, incidents, and referee assessment..." className="w-full bg-[#0B1628] border border-glass-border rounded-lg px-4 py-2.5 text-sm text-white placeholder:text-muted/60 outline-none resize-none focus:border-gold/40 transition-colors" />
                    {reportFormik.touched.content && reportFormik.errors.content && <p className="text-xs text-red-400 mt-1">{reportFormik.errors.content}</p>}
                  </div>
                  ...
                  <div className="flex justify-end gap-3 mt-6">
                    <button type="button" onClick={() => setShowAdd(false)} disabled={reportFormik.isSubmitting} className="px-5 py-2 rounded-lg text-sm text-muted border border-glass-border hover:text-white transition-colors">
                      Cancel
                    </button>
                    <button type="submit" disabled={reportFormik.isSubmitting} className="btn-gold px-6 py-2 rounded-lg text-sm font-bold disabled:opacity-50 flex items-center gap-1.5">
                      {reportFormik.isSubmitting && <Loader size={12} className="animate-spin" />}
                      Submit Report
                    </button>
                  </div>
                </form>
