...
export function RefereeHorseCheckPage() {
  ...
  const [horseChecks, setHorseChecks] = useState([]);
  ...
  const filteredChecks = horseChecks.filter((hc) => {
    ...
    const searchMatch = !search || hc.horseName?.toLowerCase().includes(query) || hc.ownerName?.toLowerCase().includes(query) || hc.jockeyName?.toLowerCase().includes(query);
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 relative min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="Horse Inspection" subtitle="Review and approve competing horse profiles by race" imageUrl="/images/hero-referee.jpg" imagePosition="right 28%" />
          ...
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            ...
            <div className="flex items-center gap-2 bg-white/[0.04] border border-glass-border rounded-lg px-3 py-2 w-full md:w-64">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search horse / owner / jockey..." className="bg-transparent text-sm text-white placeholder:text-muted/60 outline-none w-full" />
            </div>
          </div>
                      ...
                      return (
                        <tr key={hc.raceEntryId} className="hover:bg-white/[0.01] transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-semibold ${hm.ok ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                              {hm.ok ? <Heart size={11} /> : <ShieldAlert size={11} />} {hm.label}
                            </span>
                          </td>
                          <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded text-xs font-semibold ${state === 'pending' ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' : state === 'approved' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : state === 'blocked' ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>{state === 'pending' ? 'Awaiting Inspection' : state === 'approved' ? 'Approved' : state === 'blocked' ? '⚠ Ineligible (Health)' : 'Disqualified'}</span>
                          </td>
                        </tr>
                      );
                    })}
