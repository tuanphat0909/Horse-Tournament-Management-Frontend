  ...
  useEffect(() => {
    getRefereeDashboard()
      ...
      .catch((err) => {
        ...
        setError(/referee profile not found/i.test(reason) ? `${'Failed to load referee dashboard'}: ${reason}. ${'Please contact the admin to set up your referee profile (license number).'}` : `${'Failed to load referee dashboard'}: ${reason}`);
        setLoading(false);
      });
  }, []);

  const statsDisplay = [
    { title: 'Total Supervised Races', value: loading ? '...' : (data?.assignedRaceCount ?? 0), trend: 'Season 2026', icon: Flag, color: 'text-blue-400', bg: 'from-blue-500/15 to-blue-900/20', path: '/referee/schedule' },
    { title: 'Reports Pending', value: loading ? '...' : (data?.pendingReportCount ?? 0), trend: 'To Be Done', icon: ShieldCheck, color: 'text-yellow-400', bg: 'from-yellow-500/15 to-yellow-900/20', path: '/referee/horse-check' },
    { title: 'Violations Recorded', value: loading ? '...' : (data?.violationsCreatedCount ?? 0), trend: 'Needs Review', icon: AlertTriangle, color: 'text-red-400', bg: 'from-red-500/15 to-red-900/20', path: '/referee/violations' },
    { title: 'Reports Submitted', value: loading ? '...' : (data?.completedReportCount ?? 0), trend: 'Completed', icon: FileText, color: 'text-emerald-400', bg: 'from-emerald-500/15 to-emerald-900/20', path: '/referee/reports' },
  ];
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero
            title={
              <>
                {'Welcome,'} <span className="italic text-champagne">{user?.fullName ?? 'Referee'}</span>
              </>
            }
            subtitle={`${'Referee'} — ${'Season 2026'}`}
            ...
            badge={
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/10 border border-red-500/25 text-red-400 text-[10px] font-bold uppercase tracking-widest">
                <span className="w-1.5 h-1.5 rounded-full bg-red-400 animate-pulse" /> {'Monitoring Duty'}
              </div>
            }
            actions={
              <>
                <button onClick={() => navigate('/referee/horse-check')} className="btn-gold px-5 py-2 rounded-lg text-xs flex items-center gap-1.5 font-bold">
                  {'Horse Inspection'} <ShieldCheck size={13} />
                </button>
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-4 gap-4">
            {statsDisplay.map((m, i) => (
              <motion.div key={i} variants={child} onClick={() => navigate(m.path)} className="glass-panel rounded-xl p-5 relative overflow-hidden group cursor-pointer" style={{ height: '130px' }}>
          ...
          <div className="grid grid-cols-[1fr_360px] gap-6">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-5 relative z-10">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <h2 className="text-lg font-serif text-white">{'Race Schedule'}</h2>
                  <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
                </div>
                <button onClick={() => navigate('/referee/schedule')} className="text-xs text-gold hover:text-champagne flex items-center gap-1 transition-colors font-medium shrink-0 ml-3 font-bold">
                      ...
                      {data.assignedRaces.map((race) => (
                        <tr key={race.raceId} className="hover:bg-white/[0.01] transition-colors">
                          ...
                          <td className="px-4 py-3.5 text-right">
                            <button onClick={() => navigate(`/referee/horse-check`)} className="text-xs text-gold hover:underline font-semibold">
                              {'Horse Inspection'}
                            </button>
                          </td>
                        </tr>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-5 relative z-10">
                ...
                <h2 className="text-base font-serif text-white">{'Recent activity'}</h2>
                <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
              </div>
                  ...
                  {data.assignedRaces.map((race, i) => (
                    <div key={i} className="flex gap-3 items-start border-l border-glass-border pl-4 relative ml-2">
                      ...
                      <div className="space-y-1">
                        <div className="text-white font-medium">{'Race supervision assigned:'}</div>
                        <div className="text-gold font-semibold">{race.raceName}</div>
                        <div className="text-muted/60">{race.raceDate ? new Date(race.raceDate).toLocaleDateString('en-US') : ''}</div>
                      </div>
                    </div>
                  ))}
                </div>
