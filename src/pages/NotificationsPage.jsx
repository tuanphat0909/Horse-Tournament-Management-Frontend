...
import { Bell, Trophy, Activity, Sparkles, Wallet, Info, Trash2, Check, CheckSquare, ChevronLeft, ChevronRight, Eye } from 'lucide-react';
...
function categoryOf(noti) {
  ...
  if (type.includes('wallet') || type.includes('transaction') || type.includes('deposit') || type.includes('withdraw') || type.includes('payment') || type.includes('prize')) return 'wallet';
  return 'system';
}
...
export function NotificationsPage() {
  ...
  const matchesFilter = (noti, filter) => {
  ...
  const { paged: pagedNotifications, totalPages, total: totalCount, page: safePage } = paginate(visibleNotifications, currentPage, PAGE_SIZE);
  const unreadOnPage = notifications.filter((n) => !n.isRead).length;
  const filterCount = (filter) => notifications.filter((n) => matchesFilter(n, filter)).length;
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title={hero.title} subtitle={hero.subtitle} imageUrl={hero.image} imagePosition={hero.position} />
          ...
          <div className="flex flex-wrap items-center gap-2 p-1 bg-white/[0.02] border border-glass-border rounded-xl">
            {filters.map((filter) => {
              ...
              return (
                <button key={filter} onClick={() => handleFilterChange(filter)} className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all capitalize flex items-center gap-1.5 ${activeFilter === filter ? 'bg-gold text-[#0b101e] shadow-lg shadow-gold/20' : count === 0 ? 'text-muted/40 hover:text-muted hover:bg-white/[0.02]' : 'text-muted hover:text-white hover:bg-white/[0.03]'}`}>
                  {filter}
                  <span className={`text-[10px] font-bold ${activeFilter === filter ? 'opacity-70' : 'opacity-60'}`}>{count}</span>
                </button>
              );
            })}
            <button onClick={handleMarkAllRead} disabled={unreadOnPage === 0} className="ml-auto px-4 py-2 rounded-lg text-xs font-semibold text-champagne border border-glass-border hover:bg-white/[0.06] hover:border-gold/30 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center gap-1.5">
              <CheckSquare size={14} />
              Mark all as read
            </button>
          </div>
          ...
          {loading ? (
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="space-y-3">
              <AnimatePresence mode="popLayout">
                {pagedNotifications.map((noti) => (
                  <motion.div key={noti.id} variants={itemVar} exit={{ opacity: 0, x: -50 }} className={`glass-panel border rounded-xl p-4 flex gap-4 items-start relative transition-all ${!noti.isRead ? 'border-gold-border/30 bg-gold/[0.02]' : 'border-glass-border bg-white/[0.01]'}`}>
                    {!noti.isRead && <span className="absolute left-1.5 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-gold" />}

                    <div className={`p-2.5 rounded-lg border shrink-0 ${getNotiIconBg(noti.type)}`}>{getNotiIcon(noti.type)}</div>

                    <div className="flex-grow min-w-0">
                      <div className="flex items-start justify-between gap-4 mb-1">
                        <h4 className={`text-sm font-bold truncate ${!noti.isRead ? 'text-white' : 'text-white/80'}`}>{noti.title}</h4>
                        <span className="text-[10px] text-muted/60 shrink-0">{formatTime(noti.createdAt)}</span>
                      </div>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 self-center">
                      {noti.actionUrl && (
                        <button onClick={() => handleViewDetails(noti)} className="p-1.5 rounded-lg hover:bg-blue-500/10 border border-transparent hover:border-blue-500/20 text-muted hover:text-blue-400 transition-all" title="View details">
                          <Eye size={14} />
                        </button>
                      )}
                      {!noti.isRead && (
                        <button onClick={() => handleMarkRead(noti.id)} className="p-1.5 rounded-lg hover:bg-emerald-500/10 border border-transparent hover:border-emerald-500/20 text-muted hover:text-emerald-400 transition-all" title="Mark as read">
                          <Check size={14} />
                        </button>
                      )}
                      <button onClick={() => handleDelete(noti.id)} className="p-1.5 rounded-lg hover:bg-red-500/10 border border-transparent hover:border-red-500/20 text-muted hover:text-red-400 transition-all" title="Delete notification">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </motion.div>
            ...
            <div className="glass-panel border border-glass-border rounded-2xl py-16 px-4 text-center text-muted">
              ...
              <p className="text-sm font-medium">{activeFilter === 'all' ? 'No notifications yet' : `No notifications matching "${activeFilter}"`}</p>
            </div>
          )}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-glass-border/40 pt-4">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setCurrentPage(Math.max(1, safePage - 1))} disabled={safePage === 1} className="p-2 rounded-lg border border-glass-border hover:bg-white/5 disabled:opacity-40 transition-colors text-champagne">
                  <ChevronLeft size={16} />
                </button>
                <button onClick={() => setCurrentPage(Math.min(totalPages, safePage + 1))} disabled={safePage === totalPages} className="p-2 rounded-lg border border-glass-border hover:bg-white/5 disabled:opacity-40 transition-colors text-champagne">
