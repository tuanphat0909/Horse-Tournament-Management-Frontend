    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 relative min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="My Races" subtitle="Your race history and results" imageUrl="/images/hero-jockey.jpg" imagePosition="center 12%" />

          <div className="flex items-center gap-1 border-b border-glass-border pb-0">
            <button onClick={() => setTab('upcoming')} className={`px-5 py-3 text-sm font-medium border-b-2 -mb-px transition-all ${tab === 'upcoming' ? 'text-yellow-400 border-yellow-400' : 'text-muted border-transparent hover:text-white'}`}>
              Upcoming Races
              <span className={`ml-2 px-1.5 py-0.5 rounded-full text-[11px] font-bold ${tab === 'upcoming' ? 'bg-yellow-400/10 text-yellow-400' : 'bg-white/5 text-muted'}`}>{tabCounts.upcoming}</span>
            </button>
            <button onClick={() => setTab('completed')} className={`px-5 py-3 text-sm font-medium border-b-2 -mb-px transition-all ${tab === 'completed' ? 'text-emerald-400 border-emerald-400' : 'text-muted border-transparent hover:text-white'}`}>
              Completed Races
              <span className={`ml-2 px-1.5 py-0.5 rounded-full text-[11px] font-bold ${tab === 'completed' ? 'bg-emerald-400/10 text-emerald-400' : 'bg-white/5 text-muted'}`}>{tabCounts.completed}</span>
            </button>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {activeRaces.map((r, i) => {
                ...
                return (
                  <motion.div key={r.raceEntryId ?? r.raceId ?? i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} className="glass-panel rounded-2xl overflow-hidden border border-glass-border hover:border-gold/30 hover:bg-gold/[0.02] transition-all group relative">
                    <div className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-gold/40 to-transparent pointer-events-none" />
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-5 p-3 rounded-xl bg-white/[0.02] border border-glass-border hover:border-gold/30 hover:bg-gold/[0.04] transition-all w-fit flex-wrap">
                          ...
                          <div className="flex items-center gap-1 text-xs text-muted">
                            <ShieldCheck size={11} className="text-emerald-400" /> Race: <span className="text-champagne font-semibold">{r.raceName || `Race #${r.raceId}`}</span>
                          </div>
                          {(r.status?.toLowerCase() === 'completed' || r.status?.toLowerCase() === 'finished') && (
                            <>
                              ...
                              {r.finishTime && <div className="text-xs font-mono text-champagne">Time: {r.finishTime}s</div>}
                            </>
                          )}
                        </div>
                      </div>
                      <button onClick={() => setSelectedRace(r)} className="text-xs text-gold hover:text-champagne flex items-center gap-1 transition-colors shrink-0 font-medium cursor-pointer">
