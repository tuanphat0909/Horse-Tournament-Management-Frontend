...
export function JockeyDashboardPage() {
  ...
  async function handleRespond(id, status) {
    ...
    try {
      ...
      showToast(status === 'Accepted' ? 'Contract accepted successfully' : 'Contract declined successfully', `${'Contract'} #${id} ${'has been processed.'}`, 'success');
    } catch (err) {
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <PageHero
            title={
              <>
                {'Welcome,'} <span className="italic text-champagne">{user?.fullName ?? 'Jockey'}</span>
              </>
            }
            subtitle={'Manage your race invitations and racing schedule'}
            ...
            badge={
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/25 text-blue-400 text-[10px] font-bold uppercase tracking-widest">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" /> {pending.length} {'pending invitations'}
              </div>
            }
            actions={
              <>
          ...
          <div className="grid grid-cols-[1fr_380px] gap-6">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center justify-between mb-5">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <h2 className="text-lg font-serif text-white">{'Pending Invitations'}</h2>
                    ...
                    <div key={c.id ?? i} className="flex items-center gap-4 p-4 rounded-xl bg-white/[0.02] border border-yellow-500/15 hover:border-yellow-500/30 hover:bg-gold/[0.04] transition-all group">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted">
                          {'Owner:'} {c.ownerName ?? `Owner #${c.ownerId ?? '—'}`}
                      ...
                      <div className="flex gap-2 shrink-0">
                        <button disabled={respondingId === c.id} onClick={() => handleRespond(c.id, 'Accepted')} className="px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 text-xs font-bold border border-emerald-500/20 hover:bg-emerald-500/20 transition-colors disabled:opacity-50">
                          {'Accept'}
                        </button>
                        <button disabled={respondingId === c.id} onClick={() => handleRespond(c.id, 'Rejected')} className="px-3 py-1.5 rounded-lg bg-red-500/10 text-red-400 text-xs font-bold border border-red-500/20 hover:bg-red-500/20 transition-colors disabled:opacity-50">
                          {'Decline'}
                        </button>
                      </div>
                    </div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center gap-3 mb-5">
                ...
                <h2 className="text-lg font-serif text-white">{'Recent Achievements'}</h2>
                <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
              </div>
              {loading ? (
                ...
                (() => {
                  ...
                  const segments = [
                    { label: '1st Place', count: firstCount, percentage: pct1, color: '#e2ba5e', glowColor: 'rgba(226, 186, 94, 0.4)' },
                    { label: '2nd Place', count: secondCount, percentage: pct2, color: '#94a3b8', glowColor: 'rgba(148, 163, 184, 0.4)' },
                    { label: '3rd Place', count: thirdCount, percentage: pct3, color: '#b45309', glowColor: 'rgba(180, 83, 9, 0.4)' },
                    { label: 'Other', count: otherCount, percentage: pctOther, color: '#475569', glowColor: 'rgba(71, 85, 105, 0.4)' },
                  ].filter((s) => s.count > 0);
                  ...
                  return (
                    <div className="flex items-center gap-6 py-2">
                      ...
                      <div className="flex-1 space-y-1.5 min-w-0">
                        {segments.map((s, idx) => (
                          <div key={s.label} className={`flex items-center justify-between p-2 rounded-xl transition-all border ${hoveredIndex === idx ? 'bg-white/[0.04] border-white/10 scale-[1.02]' : 'bg-transparent border-transparent'}`} onMouseEnter={() => setHoveredIndex(idx)} onMouseLeave={() => setHoveredIndex(null)}>
