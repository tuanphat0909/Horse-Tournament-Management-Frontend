    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="My Violations" subtitle="Violations and appeals recorded against you" imageUrl="/images/hero-jockey.jpg" imagePosition="center 12%" />
          ...
          <div className="glass-panel rounded-xl p-4 border border-blue-500/15 bg-blue-500/[0.02] flex items-start gap-3 relative overflow-hidden">
            ...
            <div className="w-8 h-8 rounded-lg bg-blue-500/10 border border-blue-500/20 ring-1 ring-gold/20 flex items-center justify-center shrink-0 relative z-10">
              <Clock size={15} className="text-blue-400" />
            </div>
            <div className="text-xs text-muted leading-relaxed space-y-1 relative z-10">
              <div>
                When a referee records a violation, you have <span className="text-white font-bold">30 minutes</span> to submit an appeal — before the race results are officially published.
              </div>
              <div>
                The referee will review the footage and issue a final ruling. For a <span className="text-red-400 font-bold">disqualification</span>, you have an additional <span className="text-white font-bold">48 hours</span> to appeal to the organizers.
              </div>
            </div>
          </div>
                    ...
                    {violations.map((v) => (
                      <tr key={v.violationId} className="hover:bg-white/[0.01] transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="px-2.5 py-1 bg-red-500/10 text-red-400 border border-red-500/20 rounded text-xs font-semibold">{v.penalty}</span>
                        </td>
                        <td className="px-6 py-4 text-xs text-muted">{v.createdAt ? new Date(v.createdAt).toLocaleDateString('vi-VN') : ''}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
