    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          <PageHero title="Achievements" subtitle="Personal racing statistics and performance" imageUrl="/images/hero-jockey.jpg" imagePosition="center 12%" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {statsDisplay.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} className="glass-panel rounded-xl p-5 relative overflow-hidden hover:border-gold/30 transition-all group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-6">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-panel rounded-xl p-6 flex flex-col items-center justify-center text-center relative overflow-hidden">
              <div className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-gold/40 to-transparent pointer-events-none" />
              <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-gradient-to-br from-blue-500/10 to-transparent blur-[40px] pointer-events-none" />
              <div className="relative z-10 w-14 h-14 rounded-full bg-gold/10 border border-gold/25 ring-1 ring-gold/20 flex items-center justify-center mb-3">
                <Star size={28} className="text-gold" />
              </div>
              <div className="text-4xl font-serif font-bold text-white mb-1">{loading ? '—' : rankingPoint > 0 ? `#${Math.max(1, 20 - Math.floor(rankingPoint / 10))}` : 'Unranked'}</div>
                        ...
                        <div key={r.raceEntryId ?? idx} className="p-4 rounded-xl bg-white/[0.01] border border-glass-border hover:border-gold/20 flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-xs text-muted">
                              {r.horseName} • {r.tournamentName} • Lane {r.laneNo}
                            </div>
                          </div>
                          <div className="text-right">
                            <div className={`inline-block px-2 py-0.5 rounded text-xs font-bold ${r.finishPosition === 1 ? 'bg-gold/20 text-gold border border-gold/30' : r.finishPosition === 2 ? 'bg-slate-300/20 text-slate-300' : r.finishPosition === 3 ? 'bg-amber-700/20 text-amber-600' : 'bg-white/5 text-muted'}`}>Rank {r.finishPosition ?? '—'}</div>
                            {r.finishTime && <div className="text-xs font-mono text-champagne mt-1">{r.finishTime}s</div>}
                          </div>
                        </div>
                      ))}
                  </div>
